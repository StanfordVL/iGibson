...
class InteractiveIndoorScene(StaticIndoorScene):
    ...
    def __init__(
            ...
            if "category" in link.attrib:
                ...
                if category == "agent":
                    ...
                    self.agent[link.attrib["name"]] = {
                        "xyz": np.array(
                            [float(val) for val in link.attrib["xyz"].split(" ")]
                        ),
                        "rpy": np.array(
                            [float(val) for val in link.attrib["rpy"].split(" ")]
                        ),
                    }
                    continue

                if category == "multiplexer":
                    self.object_multiplexers[link.attrib["name"]][
                        "current_index"
                    ] = link.attrib["current_index"]
                    continue

                if category == "grouper":
                    self.object_groupers[link.attrib["name"]][
                        "pose_offsets"
                    ] = json.loads(link.attrib["pose_offsets"])
                    self.object_multiplexers[link.attrib["multiplexer"]][
                        "grouper"
                    ] = link.attrib["name"]
                    continue
                ...
                if (
                    self.not_load_object_categories is not None
                    and category in self.not_load_object_categories
                ):
                    continue
                ...
                # For other objects
                    ...
                    if (
                        self.load_object_categories is not None
                        and category not in self.load_object_categories
                    ):
                        continue
                    # This object is not located in one of the selected rooms, skip
                    if (
                        self.load_room_instances is not None
                        and len(set(self.load_room_instances) & set(in_rooms)) == 0
                    ):
                        continue

                    category_path = get_ig_category_path(category)
                    assert (
                        len(os.listdir(category_path)) != 0
                    ), "There are no models in category folder {}".format(category_path)
                ...
                if "bounding_box" in link.keys():
                    ...
                    scale = np.array(
                        [float(val) for val in link.attrib["scale"].split(" ")]
                    )
                else:
                ...
                joint_positions = (
                    json.loads(link.attrib["joint_positions"])
                    if "joint_positions" in link.keys()
                    else None
                )
                obj = URDFObject(
                    filename,
                ...
                if "multiplexer" in link.keys():
                    ...
                    if (
                        "object_parts"
                        not in self.object_groupers[link.attrib["grouper"]]
                    ):
                        self.object_groupers[link.attrib["grouper"]][
                            "object_parts"
                        ] = []
                    self.object_groupers[link.attrib["grouper"]]["object_parts"].append(
                        obj
                    )
                else:
                    self.add_object(obj)
        ...
        for multiplexer in self.object_multiplexers:
            ...
            grouper = self.object_groupers[
                self.object_multiplexers[multiplexer]["grouper"]
            ]
    ...
    def get_objects_with_state(self, state):
        ...
        return (
            list(self.objects_by_state[state]) if state in self.objects_by_state else []
        )
    ...
    def load_room_sem_ins_seg_map(self, seg_map_resolution):
        ...
        assert (
            img_ins.size == img_sem.size
        ), "semantic and instance seg maps have different sizes"
        self.seg_map_default_resolution = 0.01
        self.seg_map_resolution = seg_map_resolution
        self.seg_map_size = int(
            height * self.seg_map_default_resolution / self.seg_map_resolution
        )
        img_ins = np.array(
            img_ins.resize((self.seg_map_size, self.seg_map_size), Image.NEAREST)
        )
        img_sem = np.array(
            img_sem.resize((self.seg_map_size, self.seg_map_size), Image.NEAREST)
        )
    ...
    def check_scene_quality(self, body_ids, fixed_body_ids):
        ...
        for obj1_name, obj2_name in self.overlapped_bboxes:
            if (
                obj1_name not in self.objects_by_name
                or obj2_name not in self.objects_by_name
            ):
    ...
    def _load(self):
        ...
        for int_object in self.objects_by_name:
            ...
            fixed_body_ids += [
                body_id
                for body_id, is_fixed in zip(obj.body_ids, obj.is_fixed)
                if is_fixed
            ]
        ...
        for i in range(len(fixed_body_ids)):
            for j in range(i + 1, len(fixed_body_ids)):
                ...
                p.setCollisionFilterPair(
                    fixed_body_ids[i], fixed_body_ids[j], 0, 0, enableCollision=0
                )
    ...
    def seg_map_to_world(self, xy):
        ...
        return np.flip(
            (xy - self.seg_map_size / 2.0) * self.seg_map_resolution, axis=axis
        )
    ...
    def world_to_seg_map(self, xy):
        ...
        """
        return np.flip((xy / self.seg_map_resolution + self.seg_map_size / 2.0)).astype(
            np.int
        )
            ...
            if isinstance(sub_obj, ObjectGrouper):
                ...
                grouper_link.attrib = {
                    ...
                    "pose_offsets": json.dumps(
                        [(list(pos), list(orn)) for pos, orn in sub_obj.pose_offsets]
                    ),
                ...
                for group_sub_obj in sub_obj.objects:
                    ...
                    additional_attribs_by_name[group_sub_obj.name]["grouper"] = (
                        obj.name + "_grouper"
                    )
    ...
    def save_obj(self, obj, tree_root, additional_attribs_by_name):
        ...
        inv_inertial_pos, inv_inertial_orn = p.invertTransform(
            inertial_pos, inertial_orn
        )
        ...
        if link is not None:
            if obj.category == "floors":
                floor_names = [
                    obj_name
                    for obj_name in additional_attribs_by_name
                    ...
                    for key in additional_attribs_by_name[floor_name]:
                        ...
                        for floor_name in floor_names:
                            floor_mappings.append(
                                "{}:{}".format(
                                    additional_attribs_by_name[floor_name][key],
                                    floor_name,
                                )
                            )
    ...
    def save_modified_urdf(self, urdf_name, additional_attribs_by_name={}):
        ...
        for name in self.objects_by_name:
            self.save_obj_or_multiplexer(
                self.objects_by_name[name], tree_root, additional_attribs_by_name
            )
