...
class StaticIndoorScene(IndoorScene):
    ...
    def load_scene_mesh(self):
        ...
        filename = os.path.join(
            get_scene_path(self.scene_id), "mesh_z_up_downsampled.obj"
        )
        ...
        self.mesh_body_id = p.createMultiBody(
            baseCollisionShapeIndex=collision_id, baseVisualShapeIndex=visual_id
        )
    ...
    def load_floor_planes(self):
        ...
        p.resetBasePositionAndOrientation(
            floor_body_id, posObj=[0, 0, 0], ornObj=[0, 0, 0, 1]
        )
    ...
    def reset_floor(self, floor=0, additional_elevation=0.02, height=None):
        ...
        p.resetBasePositionAndOrientation(
            self.floor_body_ids[0], posObj=[0, 0, height], ornObj=[0, 0, 0, 1]
        )
