...
class IndoorScene(with_metaclass(ABCMeta, Scene)):
        ...
        for floor in range(len(self.floor_heights)):
            if self.trav_map_type == "with_obj":
                ...
                obstacle_map = np.array(
                    Image.open(os.path.join(maps_path, "floor_{}.png".format(floor)))
                )
            else:
                trav_map = np.array(
                    Image.open(
                ...
                obstacle_map = np.array(
                    Image.open(
                        os.path.join(maps_path, "floor_no_obj_{}.png".format(floor))
                    )
                )
            ...
            trav_map = cv2.erode(
                trav_map, np.ones((self.trav_map_erosion, self.trav_map_erosion))
            )
    ...
    def map_to_world(self, xy):
        ...
        return np.flip(
            (xy - self.trav_map_size / 2.0) * self.trav_map_resolution, axis=axis
        )
    ...
    def world_to_map(self, xy):
        ...
        """
        return np.flip(
            (xy / self.trav_map_resolution + self.trav_map_size / 2.0)
        ).astype(np.int)
    ...
    def get_shortest_path(self, floor, source_world, target_world, entire_path=False):
        ...
        if not g.has_node(target_map):
            ...
            g.add_edge(
                closest_node, target_map, weight=l2_distance(closest_node, target_map)
            )
        ...
        if not g.has_node(source_map):
            ...
            g.add_edge(
                closest_node, source_map, weight=l2_distance(closest_node, source_map)
            )
