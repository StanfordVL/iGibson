...
avg_category_specs = get_ig_avg_category_specs()

def generate_scene_manifest(scene_tree, scene_dir, random_groups):
    ...
    for link in scene_tree.findall("link"):
        ...
        if category == "multiplexer":
            obj_manifest.append({
                "category": category,
                "link_name": link.attrib["name"], 
                "current_idx":  link.attrib["current_index"]
            })
            continue
        ...
        if category == "grouper":
            obj_manifest.append({
                "category": category,
                "link_name": link.attrib["name"], 
                "pose_offset": json.loads(link.attrib["pose_offsets"]),
                "multiplexer": link.attrib["name"],
            })
            continue

        # An object can in mulositiple rooms, seperated by commas,
        ...
        if category in ["walls", "floors", "ceilings"]:
            filename = os.path.join(scene_dir, "urdf", model + "_" + category + ".urdf")
        ...
        joint_connecting_embedded_link = [
            joint
            for joint in scene_tree.findall("joint")
        ...
        obj_manifest.append({
            ...
            "scale": scale,
            "joint_connecting_embedded_link": joint_connecting_embedded_link,
            "filename": filename,
...
def load_overlapped_bboxes(scene_dir):
    """
    ...
    """
    bbox_overlap_file = os.path.join(scene_dir, "misc", "bbox_overlap.json")
...
class Scene:
    ...
    def __init__(self,
        ...
        self.loaded = False
        self.merge_fixed_links = True #scene_settings.merge_fixed_links
        self.category_ids = get_ig_category_ids()
        self.scene_dir = scene_dir

        self.scene_tree = ET.parse(scene_urdf)
        self.scene_manifest = generate_scene_manifest(self.scene_tree, scene_dir, random_groups=[])

        # load overlapping bboxes in scene annotation
        self.scene_tree = self.build_scene_objects(self.scene_manifest)
    ...
    def build_scene_objects(self,
        ...
        # Create the instance folder used for the generated URDFs
        ...
        for link in scene_manifest:
            print(link)
            category = link["category"]

            # Agent, multiplexer, and grouper are special cased
            if category not in ["agent", "multiplexer", "grouper"]:
                # For agents, the pose of the base link is stored in the scene
                # URDF, instead of the pose of the centroid of the bounding box
                obj = URDFObject(
                    link["filename"],
                    ...
                    category=category,
                    model_path=link["model_path"],
                    bounding_box=link["bounding_box"],
                    scale=link["scale"],
                    connecting_joint=link["joint_connecting_embedded_link"],
                    avg_obj_dims=avg_category_specs.get(category),
                    in_rooms=link["in_rooms"],
                    texture_randomization=texture_randomization,
                    overwrite_inertial=True,
                    scene_instance_folder=self.scene_instance_folder,
                    bddl_object_scope=link["bddl_object_scope"],
                    joint_positions=link["joint_positions"],
                ...
                # Load object states.
                if link["state_cache"]:
                    for state_name, state_dump in link["state_cache"].items():
                        state_class = get_state_from_name(state_name)
                        if state_class in obj.states:
                            obj.states[state_class].load(state_dump)

                if "multiplexer" in link:
                    self.object_multiplexers[link["multiplexer"]]["whole_object"] = obj

                elif "grouper" in link:
                    if "object_parts" not in self.object_groupers[link["grouper"]]:
                        self.object_groupers[link["groupoer"]]["object_parts"] = []
                    self.object_groupers[link["grouper"]]["object_parts"].append(obj)
                else:
                    self.add_object(obj)
            else:
                if category == "agent":
                    # For agents, the pose of the base link is stored in the scene
                    # URDF, instead of the pose of the centroid of the bounding box
                    self.agent[link["name"]] = {
                        "xyz": np.array([float(val) for val in link["xyz"].split(" ")]),
                        "rpy": np.array([float(val) for val in link["rpy"].split(" ")]),
                    }

                elif category == "multiplexer":
                    self.object_multiplexers[link["name"]]["current_index"] = link["current_index"]
                    continue

                elif category == "grouper":
                    self.object_groupers[link["name"]]["pose_offsets"] = link["pose_offsets"]
                    self.object_multiplexers[link["multiplexer"]]["grouper"] = link["name"]
                    continue
    ...
    def load(self):
        ...
        bboxes = load_overlapped_bboxes(self.scene_dir)
        self.check_scene_quality(body_ids, fixed_body_ids, bboxes)
    ...
    def save(self, urdf_name, additional_attribs_by_name={}):
        """
        ...
        """
        scene_tree = ET.parse(self.scene_urdf)
    ...
    def check_scene_quality(self, body_ids, fixed_body_ids, bboxes):
        """
        ...
        overlapped_body_ids = []
        for obj1_name, obj2_name in bboxes:
