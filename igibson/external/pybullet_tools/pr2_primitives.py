...
class GripperCommand(Command):
    ...
    def __repr__(self):
        return "{}({},{},{})".format(
            self.__class__.__name__, get_body_name(self.robot), self.arm, self.position
        )
...
class Attach(Command):
    ...
    def apply(self, state, **kwargs):
        state.attachments[self.body] = create_attachment(
            self.robot, self.link, self.body
        )
...
class Clean(Command):
    ...
    def control(self, **kwargs):
        p.addUserDebugText(
            ...
            textColorRGB=(0, 0, 1),  # textSize=1,
            lifeTime=0,
            parentObjectUniqueId=self.body,
        )
...
def get_grasp_gen(problem, collisions=False, randomize=True):
    ...
    def fn(body):
        ...
        for grasp in grasps:
            grasp_width = (
                compute_grasp_width(problem.robot, arm, body, grasp.value)
                if collisions
                else 0.0
            )
...
def iterate_approach_path(robot, arm, gripper, pose, grasp, body=None):
    ...
    tool_link = link_from_name(robot, PR2_TOOL_FRAMES[arm])
    tool_from_root = multiply(
        invert(get_link_pose(robot, tool_link)), get_link_pose(robot, root_link)
    )
...
def get_ir_sampler(
    problem, custom_limits={}, max_attempts=25, collisions=True, learned=True
):
    robot = problem.robot
    obstacles = problem.fixed if collisions else []
    ...
    def gen_fn(arm, obj, pose, grasp):
        ...
        for _ in iterate_approach_path(robot, arm, gripper, pose, grasp, body=obj):
            if any(
                pairwise_collision(gripper, b) or pairwise_collision(obj, b)
        ...
        if learned:
            base_generator = learned_pose_generator(
                robot, gripper_pose, arm=arm, grasp_type=grasp.grasp_type
            )
        else:
            base_generator = uniform_pose_generator(robot, gripper_pose)
        lower_limits, upper_limits = get_custom_limits(
            robot, base_joints, custom_limits
        )
        while True:
            count = 0
...
def get_ik_fn(problem, custom_limits={}, collisions=True, teleport=False):
    ...
    def fn(arm, obj, pose, grasp, base_conf):
        ...
        open_arm(robot, arm)
        set_joint_positions(
            robot, arm_joints, default_conf
        )  # default_conf | sample_fn()
        ...
        cmd = Commands(
            State(attachments=attachments), savers=[BodySaver(robot)], commands=[mt]
        )
        return (cmd,)
...
def get_motion_gen(problem, custom_limits={}, collisions=True, teleport=False):
    ...
    def fn(bq1, bq2):
        ...
        if teleport:
            ...
            raw_path = plan_base_motion(
                robot, goal_conf, BASE_LIMITS, obstacles=obstacles
            )
            if raw_path is None:
                print("Failed motion plan!")
                return None
    ...
    def gen(arm, button):
        ...
        while True:
            for _ in range(max_attempts):
                ...
                gripper_pose = multiply(
                    pose, invert(press_pose)
                )  # w_f_g = w_f_o * (g_f_o)^-1
                # approach_pose = gripper_pose # w_f_g * g_f_o * o_f_a = w_f_a
                approach_pose = multiply(
                    gripper_pose, invert(multiply(press_pose, approach))
                )

                if learned:
                    base_generator = learned_pose_generator(
                        robot, gripper_pose, arm=arm, grasp_type=grasp_type
                    )
                else:
                    base_generator = uniform_pose_generator(robot, gripper_pose)
                ...
                approach_movable_conf = sub_inverse_kinematics(
                    robot, joints[0], link, approach_pose
                )
                # approach_movable_conf = inverse_kinematics(robot, link, approach_pose)
                if (approach_movable_conf is None) or any(
                    pairwise_collision(robot, b) for b in fixed
                ):
                    continue
                approach_conf = get_joint_positions(robot, joints)

                gripper_movable_conf = sub_inverse_kinematics(
                    robot, joints[0], link, gripper_pose
                )
                # gripper_movable_conf = inverse_kinematics(robot, link, gripper_pose)
                if (gripper_movable_conf is None) or any(
                    pairwise_collision(robot, b) for b in fixed_wo_button
                ):
                    continue
                ...
                if teleport:
                    ...
                    control_path = plan_direct_joint_motion(
                        robot,
                        joints,
                        approach_conf,
                        obstacles=fixed_wo_button,
                        self_collisions=SELF_COLLISIONS,
                    )
                    if control_path is None:
                        continue
                    set_joint_positions(robot, joints, approach_conf)
                    retreat_path = plan_joint_motion(
                        robot,
                        joints,
                        default_conf,
                        obstacles=fixed,
...
class State(object):
    def __init__(self, attachments={}, cleaned=set(), cooked=set()):
        self.poses = {
            body: Pose(body, get_pose(body))
            for body in get_bodies()
            if body not in attachments
        }
        self.grasps = {}
