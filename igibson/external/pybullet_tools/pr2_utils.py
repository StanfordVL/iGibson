...
PR2_GROUPS = {
    ...
    gripper_from_arm("left"): [
        "l_gripper_l_finger_joint",
        "l_gripper_r_finger_joint",
        "l_gripper_l_finger_tip_joint",
        "l_gripper_r_finger_tip_joint",
    ],
    gripper_from_arm("right"): [
...
TOOL_POSE = Pose(
    euler=Euler(pitch=np.pi / 2)
)  # l_gripper_tool_frame (+x out of gripper arm)
...
def load_dae_collisions():
    ...
    ignore_regex = (
        r'<\s*ignore_link_pair\s+link0="kmodel1/(\w+)"\s+link1="kmodel1/(\w+)"\s*/>'
    )
    disabled_collisions = []
...
def load_srdf_collisions():
    ...
    regex = (
        r'<\s*disable_collisions\s+link1="(\w+)"\s+link2="(\w+)"\s+reason="(\w+)"\s*/>'
    )
    disabled_collisions = []
...
def get_cylinder_push(
    ...
    translate_x = Pose(
        point=[-diameter / 2 - side_offset, 0, 0]
    )  # Compute as a function of theta
...
def get_x_presses(
    body, max_orientations=1, body_pose=unit_pose(), top_offset=PRESS_OFFSET
):
    # gripper_from_object
    # TODO: update
    ...
    for j in range(max_orientations):
        ...
        press_poses += [
            multiply(TOOL_POSE, translate, swap_xz, translate_center, body_pose)
        ]
    return press_poses


def get_top_presses(
    body, tool_pose=TOOL_POSE, body_pose=unit_pose(), top_offset=PRESS_OFFSET, **kwargs
):
...
PR2_CAMERA_MATRIX = get_camera_matrix(width=640, height=480, fx=772.55, fy=772.5)
...
def get_pr2_field_of_view(**kwargs):
    ...
    horizontal = angle_between(
        [view_lower[0], 0, z], [view_upper[0], 0, z]
    )  # 0.7853966439794928
    vertical = angle_between(
        [0, view_lower[1], z], [0, view_upper[1], z]
    )  # 0.6024511557247721
...
def is_visible_aabb(aabb, **kwargs):
    ...
    return not (
        np.any(body_lower[:2] < view_lower[:2])
        or np.any(view_upper[:2] < body_upper[:2])
    )
...
def attach_viewcone(
    robot,
    head_name=HEAD_LINK_NAME,
    depth=MAX_VISUAL_DISTANCE,
    camera_matrix=None,
    color=(1, 0, 0),
    **kwargs
):
    ...
    for v1, v2 in cone_wires_from_support(
        ...
        if is_optical(head_name):
            ...
            rotation = Pose(
                euler=Euler(roll=-np.pi / 2, yaw=-np.pi / 2)
            )  # Apply in reverse order
        p1 = tform_point(rotation, v1)
        p2 = tform_point(rotation, v2)
        lines.append(
            add_line(p1, p2, color=color, parent=robot, parent_link=head_link, **kwargs)
        )
    return lines


def draw_viewcone(
    pose, depth=MAX_VISUAL_DISTANCE, camera_matrix=None, color=(1, 0, 0), **kwargs
):
    lines = []
    for v1, v2 in cone_wires_from_support(
        get_viewcone_base(depth=depth, camera_matrix=camera_matrix)
    ...
    with LockRenderer():
        with ConfSaver(pr2):
            for _ in range(max_iterations):
                ...
                normal_world = tform_point(
                    (unit_point(), quat_from_pose(world_from_head)), normal_head
                )
                correction_quat = quat_from_axis_angle(
                    normal_world, step_size * error_angle
                )
                correction_euler = euler_from_quat(correction_quat)
                _, angular = compute_jacobian(pr2, head_link)
                correction_conf = np.array(
                    [
                        np.dot(angular[mj], correction_euler)
...
def plan_pause_scan_path(pr2, tilt=0):
    ...
    return [
        np.array([pan, tilt])
        for pan in np.linspace(
            lower_limit + epsilon, upper_limit - epsilon, n, endpoint=True
        )
    ]
...
def get_detections(
    ...
    **kwargs
    ...
    for body in get_bodies():
        ...
        if not any(
            pairwise_collision(cone, obst)
            for obst in set(get_bodies()) - {pr2, body, cone}
        ) and not any(
            link_pairs_collision(pr2, [link], cone)
...
def get_visual_detections(pr2, **kwargs):
    return [
        body for body, _ in get_detections(pr2, depth=MAX_VISUAL_DISTANCE, **kwargs)
    ]


def get_kinect_registrations(pr2, **kwargs):
    return [
        body for body, _ in get_detections(pr2, depth=MAX_KINECT_DISTANCE, **kwargs)
    ]
...
def visible_base_generator(robot, target_point, base_range):
    ...
    while True:
        ...
        base_theta = np.math.atan2(
            base_from_table[1], base_from_table[0]
        )  # TODO: stochastic orientation?
...
def create_gripper(robot, arm, visual=True):
    ...
    gripper = clone_body(
        robot, links=links, visual=False, collision=True
    )  # TODO: joint limits
    if not visual:
