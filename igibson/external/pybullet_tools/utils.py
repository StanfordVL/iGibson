...
def fnc_with_client(fnc, client, *args, **kwargs):
    prev_client = get_client()
    set_client(client)
    retval = fnc(*args, **kwargs)
    set_client(prev_client)
    return retval
...
def load_pybullet(filename, fixed_base=False, scale=1.0, **kwargs):
    ...
    with LockRenderer():
        if filename.endswith(".urdf"):
            ...
            body = p.loadURDF(
                filename, useFixedBase=fixed_base, flags=flags, globalScaling=scale, physicsClientId=CLIENT
            )
...
def set_camera_pose(camera_point, target_point=np.zeros(3)):
    ...
    p.resetDebugVisualizerCamera(distance, math.degrees(yaw), math.degrees(pitch), target_point, physicsClientId=CLIENT)
...
def get_image(
    ...
    view_matrix = p.computeViewMatrix(
        cameraEyePosition=camera_pos, cameraTargetPosition=target_pos, cameraUpVector=[0, 0, 1], physicsClientId=CLIENT
    )
...
def quat_angle_between(quat0, quat1):  # quaternion_slerp
    ...
    d = clip(np.dot(q0, q1), min_value=-1.0, max_value=+1.0)
    angle = math.acos(d)
    # TODO: angle_between
...
def dump_body(body):
    ...
    print(
        "Link id: {} | Name: {} | Mass: {} | Collision: {} | Visual: {}".format(
            link, get_base_name(body), get_mass(body), len(get_collision_data(body, link)), -1
        )
...
def get_labeled_configuration(body):
    ...
    return dict(zip(get_joint_names(body, movable_joints), get_joint_positions(body, movable_joints)))
...
def wrap_positions(body, joints, positions):
    ...
    return [wrap_position(body, joint, position) for joint, position in zip(joints, positions)]
...
def get_link_state(body, link, kinematics=True, velocity=True):
    ...
    return LinkState(
        *p.getLinkState(
            body, link, physicsClientId=CLIENT  # computeLinkVelocity=velocity, computeForwardKinematics=kinematics,
        )
    )
...
def are_links_adjacent(body, link1, link2):
    return (get_link_parent(body, link1) == link2) or (get_link_parent(body, link2) == link1)
...
def get_adjacent_fixed_links(body):
    return list(filter(lambda item: not is_movable(body, item[0]), get_adjacent_links(body)))
...
def get_box_geometry(width, length, height):
    return {"shapeType": p.GEOM_BOX, "halfExtents": [width / 2.0, length / 2.0, height / 2.0]}
...
def create_body(collision_id=-1, visual_id=-1, mass=STATIC_MASS):
    return p.createMultiBody(
        baseMass=mass, baseCollisionShapeIndex=collision_id, baseVisualShapeIndex=visual_id, physicsClientId=CLIENT
    )
...
def create_obj(path, scale=1.0, mass=STATIC_MASS, collision=True, color=(0.5, 0.5, 0.5, 1)):
...
CollisionShapeData = namedtuple(
    "CollisionShapeData",
    ["object_unique_id", "linkIndex", "geometry_type", "dimensions", "filename", "local_frame_pos", "local_frame_orn"],
)
...
def get_data_filename(data):
    return (data.filename if isinstance(data, CollisionShapeData) else data.meshAssetFileName).decode(encoding="UTF-8")
...
def body_collision(body1, body2, max_distance=MAX_DISTANCE):  # 10000
    ...
    return (
        len(p.getClosestPoints(bodyA=body1, bodyB=body2, distance=max_distance, physicsClientId=CLIENT)) != 0
    )  # getContactPoints`
...
RayResult = namedtuple("RayResult", ["objectUniqueId", "linkIndex", "hit_fraction", "hit_position", "hit_normal"])
...
def get_self_link_pairs(body, joints, disabled_collisions=set(), only_moving=True):
    ...
    check_link_pairs = list(
        filter(
            lambda pair: (pair not in disabled_collisions) and (pair[::-1] not in disabled_collisions), check_link_pairs
        )
    )
...
def get_collision_fn(
    ...
    **kwargs
    ...
    print("Moving links: ", moving_links)
...
def plan_joint_motion(
    ...
    **kwargs
    ...
    if not check_initial_end(start_conf, end_conf, collision_fn):
        print("Initial or final configuration are in collision")
        return None
    if algorithm == "direct":
        return direct_path(start_conf, end_conf, extend_fn, collision_fn)
    elif algorithm == "birrt":
        print("Planing arm birrt")
        return birrt(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs)
    elif algorithm == "rrt_star":
        return rrt_star(
            start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, max_iterations=5000, **kwargs
        )
    elif algorithm == "rrt":
        return rrt(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, iterations=500, **kwargs)
    elif algorithm == "lazy_prm":
        return lazy_prm_replan_loop(
...
def get_closest_angle_fn(body, joints, linear_weight=1.0, angular_weight=1.0, reversible=True):
    assert len(joints) == 3
...
def plan_base_motion_2d(
    ...
    **kwargs
    ...
    def collision_fn(q):
        ...
        if (
            pts[0] < robot_footprint_radius_in_map
            or pts[1] < robot_footprint_radius_in_map
            or pts[0] > grid_resolution - robot_footprint_radius_in_map - 1
            or pts[1] > grid_resolution - robot_footprint_radius_in_map - 1
        ):
            return True
        ...
        cv2.circle(
            mask, (robot_footprint_radius_in_map, robot_footprint_radius_in_map), robot_footprint_radius_in_map, 1, -1
        )
...
def sample_placement_on_aabb(top_body, bottom_aabb, top_pose=unit_pose(), percent=1.0, max_attempts=50, epsilon=1e-3):
...
def get_constraints():
    ...
    """
    return [
        p.getConstraintUniqueId(i, physicsClientId=CLIENT) for i in range(p.getNumConstraints(physicsClientId=CLIENT))
    ]
...
def get_constraint_violation(cid):
    ...
    joint_pos_in_child_world = p.multiplyTransforms(child_link_pos, child_link_orn, joint_position_child, [0, 0, 0, 1])[
        0
    ]
...
class Attachment(object):
    ...
    @property
    def bodies(self):
        return flatten_links(self.child) | flatten_links(self.parent, get_link_subtree(self.parent, self.parent_link))
...
def joint_controller_hold2(
    body, joints, positions, velocities=None, tolerance=1e-2 * np.pi, position_gain=0.05, velocity_gain=0.01
):
    """
    Keeps other joints in place
    """
...
def velocity_control_joints(body, joints, velocities):
    # kv = 0.3
    return p.setJointMotorControlArray(
        body, joints, p.VELOCITY_CONTROL, targetVelocities=velocities, physicsClientId=CLIENT
    )  # ,
    # velocityGains=[kv] * len(joints),)
    # forces=forces)
...
def compute_jacobian(robot, link, positions=None):
    ...
    translate, rotate = p.calculateJacobian(
        robot, link, unit_point(), positions, velocities, accelerations, physicsClientId=CLIENT
    )
...
def interpolate_poses(pose1, pose2, pos_step_size=0.01, ori_step_size=np.pi / 16):
    ...
    pos2, quat2 = pose2
    num_steps = int(
        math.ceil(max(get_distance(pos1, pos2) / pos_step_size, quat_angle_between(quat1, quat2) / ori_step_size))
    )
    for i in range(num_steps):
...
def plan_cartesian_motion(
    robot, first_joint, target_link, waypoint_poses, max_iterations=200, custom_limits={}, **kwargs
):
...
def get_aabb_vertices(aabb):
    d = len(aabb[0])
    return [tuple(aabb[i[k]][k] for k in range(d)) for i in product(range(len(aabb)), repeat=d)]
...
def is_mesh_on_surface(polygon, world_from_surface, mesh, world_from_mesh, epsilon=1e-2):
    ...
    return (abs(min_z) < epsilon) and all(is_point_in_polygon(p, polygon) for p in points_surface)
...
def convex_centroid(vertices):
    ...
    return sum((v1 + v2) * np.cross(v1, v2) for v1, v2 in segments) / (6.0 * convex_signed_area(vertices))
...
def is_collision_free(body_a, link_a_list, body_b=None, link_b_list=None):
    ...
    if body_b is None:
        for link_a in link_a_list:
            contact_pts = p.getContactPoints(bodyA=body_a, linkIndexA=link_a, physicsClientId=CLIENT)
            if len(contact_pts) > 0:
                print("Collision in body {} link {}".format(get_body_name(body_a), get_link_name(body_a, link_a)))
                return False
    elif link_b_list is None:
        for link_a in link_a_list:
            contact_pts = p.getContactPoints(bodyA=body_a, bodyB=body_b, linkIndexA=link_a, physicsClientId=CLIENT)
            if len(contact_pts) > 0:
                return False
    else:
        for link_a in link_a_list:
            for link_b in link_b_list:
                contact_pts = p.getContactPoints(
                    bodyA=body_a, bodyB=body_b, linkIndexA=link_a, linkIndexB=link_b, physicsClientId=CLIENT
                )
                if len(contact_pts) > 0:
                    return False
    return True
