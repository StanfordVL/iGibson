...
class VoxelGrid(object):
    ...
    def center_from_voxel(self, voxel):
        return self.lower_from_voxel(voxel) + self.resolutions / 2.0
    ...
    def check_collision(self, box, voxel, pairs, threshold=0.0):
        ...
        return any(
            pairwise_link_collision(body1, link1, body2, link2, max_distance=threshold)
    ...
    def draw_voxel_bodies(self):
        ...
        with LockRenderer():
            ...
            for voxel in sorted(self.occupied):
                handles.extend(
                    draw_aabb(self.aabb_from_voxel(voxel), color=self.color[:3])
                )
            return handles
    ...
    def create_height_map(
        ...
        plane_lower = get_point(plane) - plane_extent / 2.0
