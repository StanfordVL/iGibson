...
def sample_tool_ik(
    robot, arm, tool_pose, nearby_conf=USE_ALL, max_attempts=25, **kwargs
):
...
def pr2_inverse_kinematics(
    robot, arm, gripper_pose, obstacles=[], custom_limits={}, **kwargs
):
    arm_link = get_gripper_link(robot, arm)
    arm_joints = get_arm_joints(robot, arm)
    if is_ik_compiled():
        ik_joints = get_torso_arm_joints(robot, arm)
        torso_arm_conf = sample_tool_ik(
            robot,
            arm,
            gripper_pose,
        ...
        arm_conf = sub_inverse_kinematics(
            robot, arm_joints[0], arm_link, gripper_pose, custom_limits=custom_limits
        )
        if arm_conf is None:
            return None
