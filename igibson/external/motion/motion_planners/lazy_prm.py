...
def unit_cost_fn(v1, v2):
    return 1.0


def zero_heuristic_fn(v):
    return 0
...
def lazy_prm(
    ...
    def cost_fn(v1, v2):
        return distance_fn(samples[v1], samples[v2])
    ...
    def neighbors_fn(v1):
        for v2 in neighbors_from_index[v1]:
            if not (
                colliding_vertices.get(v2, False)
                or colliding_edges.get((v1, v2), False)
            ):
                yield v2
    ...
    def heuristic_fn(v):
        return visited[v].g if v in visited else INF
