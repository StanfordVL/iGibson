...
class SAViRTTask(PointNavRandomTask):
    ...
    def sample_initial_pose_and_target_pos(self, env):
        ...
        # initial_orn = np.array([0, 0, np.pi /2])
...
#                     env.robots[0], target_pos)
    ...
    def load_gt_rt_map(self, env):
        if self.config["scene"] == "igibson":
            ...
            ######
            H, W = gt_rt.shape
            local_map = gt_rt[int(H/2-100):int(H/2),int(W/2 - 50):int(W/2 + 50)]
            cv2.imwrite("local_map.png", local_map)
...
class SAViOccTask(PointNavRandomTask):
    # reward function
    def __init__(self, env):
        super().__init__(env)
        self.reward_functions = [
    ...
    def sample_initial_pose_and_target_pos(self, env):
        ...
        for i in range(max_trials):
            ...
            if self.config['scene'] == "igibson":
                ...
                if len(target_poses) != 0:
                    ...
                    break
        if not set_target:
            print("scene that fail to set target", env.scene.scene_id)
            logging.warning("WARNING: Failed to set target")
        ...
        if not (self.target_dist_min < dist < self.target_dist_max):
            print("scene that fail to sample positions!", env.scene.scene_id)
            logging.warning("WARNING: Failed to sample initial and target positions")
        initial_orn = np.array([0, 0, np.random.uniform(0, np.pi * 2)])
        initial_pos = np.array([-2.7, 10.6, 0.1])
        return initial_pos, initial_orn, target_pos
    ...
    def reset_agent(self, env, train=True):
        ...
        if train:
            ...
            if self.config["test_setting"] == "val":
                env.audio_system.registerSource(self.audio_obj_id, self.config['audio_dir'] \
                                                +"/val/"+self.cat+".wav", enabled=True)
            elif self.config["test_setting"] == "test_unheard":
                env.audio_system.registerSource(self.audio_obj_id, self.config['audio_dir'] \
                                                +"/test_unheard/"+self.cat+".wav", enabled=True)
            elif self.config["test_setting"] == "test_heard":
                env.audio_system.registerSource(self.audio_obj_id, self.config['audio_dir'] \
                                                +"/test_heard/"+self.cat+".wav", enabled=True)     
...
if __name__ == "__main__":
    ...
    config = "C:/Users/capri28/Documents/iGibson-dev/igibson/agents/savi_rt/config/savi_rt_step1.yaml"
    ...
    depth = obs["depth"]
    top_down = obs["top_down_video"]
    print(depth.shape)
    cv2.imwrite("depth.png", (depth *255).astype(np.uint8))
    cv2.imwrite("top_down.png", top_down)
    env.close()
