...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", "-c", help="which config file to use [default: use yaml files in /configs]", required=True
    )
    ...
    if env_config_path is None:
        raise ValueError("You must specify the .yaml configuration file" " for the task environment")
    ...
    dataset_split = {
        ...
        "test": ["Pomaria_2_int", "Benevolence_2_int", "Beechwood_0_int", "Pomaria_1_int", "Merom_1_int"],
    }
    ...
    for split in dataset_split:
        for scene_id in dataset_split[split]:
            ...
            # Load the simulator
            env = iGibsonEnv(
                config_file=env_config, mode="headless", action_timestep=1.0 / 10.0, physics_timestep=1.0 / 40.0
            )
            ...
            for episode_index in range(raw_num_episodes):
                ...
                episode_info["initial_pos"] = list(env.robots[0].get_position())
                episode_info["initial_orn"] = list(env.robots[0].get_orientation())
                episode_info["target_pos"] = list(env.task.target_pos)

                for i in range(env.task.num_pedestrians):
                    episode_info["pedestrians"][i]["initial_pos"] = list(env.task.pedestrians[i].get_position())
                    episode_info["pedestrians"][i]["initial_orn"] = list(env.task.pedestrians[i].get_orientation())
                    for step in range(episode_length):
                        _, target_pos = env.scene.get_random_point(floor=env.task.floor_num)
                        episode_info["pedestrians"][i]["target_pos"][step] = list(target_pos)
            env.close()
            ...
            env_config["scene_episode_config_name"] = os.path.join(
                os.path.dirname(igibson.__file__), "episodes", "data", "social_nav", file_name
            )
            env = iGibsonEnv(
                config_file=env_config, mode="headless", action_timestep=1.0 / 10.0, physics_timestep=1.0 / 40.0
            )
            ...
            for i in range(raw_num_episodes):
                ...
                shortest_path, _ = env.scene.get_shortest_path(
                    env.task.floor_num, env.task.initial_pos[:2], env.task.target_pos[:2], entire_path=True
                )
                ...
                if np.linalg.norm(shortest_path[-1] - env.task.target_pos[:2]) > 0.01:
                    shortest_path = np.vstack([shortest_path, env.task.target_pos[:2]])
                ...
                for step in range(episode_length):
                    ...
                    desired_vel = desired_vel / np.linalg.norm(desired_vel) * env.task.orca_max_speed
                    env.task.orca_sim.setAgentPrefVelocity(env.task.robot_orca_ped, tuple(desired_vel))
                    env.task.step(env)
                    pos = env.task.orca_sim.getAgentPosition(env.task.robot_orca_ped)
                    env.robots[0].set_position([pos[0], pos[1], 0.0])
