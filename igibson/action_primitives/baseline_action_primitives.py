"""
WARNING!
The BaselineActionPrimitive is a work-in-progress and is only provided as an example.
...
class BaselineActionPrimitive(IntEnum):
    GRASP = 0
    PLACE_ON_TOP = 1
    PLACE_INSIDE = 2
    OPEN = 3
...
class BaselineActionPrimitives(BaseActionPrimitiveSet):
    def __init__(self, task, scene, robot):
        ...
        self.controller_functions = {
            BaselineActionPrimitive.GRASP: self.grasp,
            BaselineActionPrimitive.PLACE_ON_TOP: self.place_on_top,
            BaselineActionPrimitive.PLACE_INSIDE: self.place_inside,
            BaselineActionPrimitive.OPEN: self.open,
            BaselineActionPrimitive.CLOSE: self.close,
            BaselineActionPrimitive.NAVIGATE_TO: self._navigate_to_obj,
        }
        self.arm = "right_hand"
    ...
    def get_action_space(self):
        ...
        return gym.spaces.Tuple(
            [gym.spaces.Discrete(self.num_objects), gym.spaces.Discrete(len(BaselineActionPrimitive))]
        )

    def get_action_from_primitive_and_object(self, primitive: BaselineActionPrimitive, obj: BaseObject):
    ...
    def apply(self, action):
        ...
        action = BaselineActionPrimitive(action_idx)
    ...
    def grasp(self, obj, sticky=True):
