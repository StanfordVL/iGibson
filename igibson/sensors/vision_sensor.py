...
class VisionSensor(BaseSensor):
    ...
    def __init__(self, env, modalities):
        ...
        if "rgb_filled" in modalities:
            ...
            self.comp.load_state_dict(
                torch.load(os.path.join(igibson.assets_path, "networks", "model.pth"))
            )
            self.comp.eval()
    ...
    def get_ins_seg(self, raw_vision_obs):
        ...
        :return: semantic segmentation mask, normalized to [0.0, 1.0]
        """
        seg = (raw_vision_obs["ins_seg"][:, :, 0:1] * MAX_INSTANCE_COUNT).astype(
            np.int32
        )
        return seg
    ...
    def get_obs(self, env):
        ...
        if isinstance(env.robots[0], BehaviorRobot):
            raw_vision_obs = env.robots[0].render_camera_image(
