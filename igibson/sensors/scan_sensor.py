...
class ScanSensor(BaseSensor):
    ...
    def get_local_occupancy_grid(self, scan):
        ...
        occupancy_grid = np.zeros((self.grid_resolution, self.grid_resolution)).astype(
            np.uint8
        )

        occupancy_grid.fill(int(OccupancyGridState.UNKNOWN * 2.0))
        scan_local_in_map = scan_local / self.occupancy_range * self.grid_resolution + (
