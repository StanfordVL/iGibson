...
class ViewerVR:
    ...
    def update(self):
        ...
        if self.frame_save_path:
            frame = cv2.cvtColor(
                self.renderer.render(return_frame=True)[0], cv2.COLOR_RGB2BGR
            )
            ...
            # Save as a folder of images
                ...
                final_save_path = os.path.join(
                    self.frame_save_path, "%05d.jpg" % self.frame_counter
                )
...
class Viewer:
    def __init__(
        ...
        self.phi = np.arctan2(
            ...
            np.sqrt(initial_view_direction[0] ** 2 + initial_view_direction[1] ** 2),
        )
    ...
    def monitor_constraint_violation(self):
        for cid in self.cid:
            ...
            if child_link == -1:
                ...
                child_link_pos, child_link_orn = p.getLinkState(child_body, child_link)[
                    :2
                ]
    ...
    def move_constraint(self, x, y):
        ...
        position_cam[:3] = (
            position_cam[:3] / np.linalg.norm(position_cam[:3]) * self.dist
        )
        position_world = np.linalg.inv(self.renderer.V).dot(position_cam)
    ...
    def mouse_callback(self, event, x, y, flags, params):
        ...
        if self.manipulation_mode == 0:
            ...
            if (
                flags == cv2.EVENT_FLAG_LBUTTON + cv2.EVENT_FLAG_CTRLKEY
        ...
        # Manipulation mode
            ...
            if event == cv2.EVENT_MOUSEMOVE:  # moving mouse location on the window
                if (
                    self.left_down or self.middle_down
    ...
    def show_help_text(self, frame):
        ...
        if self.show_help >= 150:
            ...
            help_text = (
                "'m': switch control mode across navigation, manipulation, and planning"
            )
            cv2.putText(
                frame,
                help_text,
            ...
            help_text = (
                "Middle click and drag (linux) or left SHIFT + left click and drag:"
            )
            cv2.putText(
                frame,
                help_text,
    ...
    def update(self):
        ...
        if self.recording and not self.pause_recording:
            cv2.imwrite(
                os.path.join(self.video_folder, "{:05d}.png".format(self.frame_idx)),
