...
class Material(object):
    ...
    def __str__(self):
        return "Material(material_type: {}, texture_id: {}, metallic_texture_id:{}, roughness_texture_id:{}, " "normal_texture_id:{}, color: {})".format(
            self.material_type,
            self.texture_id,
...
class ProceduralMaterial(Material):
    ...
    def update(self):
        if len(self.requests) == 0:
            ...
            requests = sorted(
                self.requests, key=lambda state: TEXTURE_CHANGE_PRIORITY[state]
            )
...
class RandomizedMaterial(Material):
    ...
    def get_material_files(self):
        ...
        for material_class in self.material_classes:
            ...
            assert material_class in all_materials, "unknown material class: {}".format(
                material_class
            )
