from igibson.render.mesh_renderer.mesh_renderer_cpu import (
    MeshRenderer,
...
class VrHUDOverlay(VrOverlayBase):
    ...
    def __init__(self, overlay_name, renderer, width=1, pos=[0, 0, -1]):
        ...
        self.renderer.vrsys.createOverlay(
            self.overlay_name, self.width, self.pos[0], self.pos[1], self.pos[2], ""
        )
...
class VrSettings(object):
    ...
    def __init__(self, use_vr=False, config_str=None):
        ...
        self.vr_config_path = os.path.join(
            mesh_renderer_folder, "..", "..", "vr_config.yaml"
        )
    ...
    def load_vr_config(self, config_str=None):
        ...
        assert self.relative_movement_device in [
            "hmd",
            "left_controller",
            "right_controller",
        ]
        ...
        self.store_only_first_event_per_button = shared_settings[
            "store_only_first_event_per_button"
        ]
...
class MeshRendererVR(MeshRenderer):
    ...
    def __init__(
        ...
        self.vr_rendering_settings.show_glfw_window = (
    ...
    def gen_static_overlay(self, image_fpath, width=1, pos=[0, 0, -1]):
        ...
        """
        uniq_name = "overlay{}".format(time.perf_counter())
        static_overlay = VrStaticImageOverlay(
            uniq_name, self, image_fpath, width=width, pos=pos
        )
    ...
    def render(self, return_frame=False):
        ...
        if self.vr_settings.use_vr:
            ...
            super().render(
                modes=("rgb"),
                return_buffer=False,
                render_shadow_pass=False,
                render_text_pass=False,
            )
            ...
            if return_frame:
                return super().render(
                    modes=("rgb"), return_buffer=return_frame, render_shadow_pass=True
                )
            else:
                super().render(
                    modes=("rgb"), return_buffer=False, render_shadow_pass=True
                )
