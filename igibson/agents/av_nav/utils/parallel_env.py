...
class ParallelNavEnv(iGibsonEnv):
    ...
    def step(self, actions, train, scene_splits=None):
        """Forward a batch of actions to the wrapped environments.

        :param actions: batched action, possibly nested, to apply to the environment.
        :return: a list of [next_obs, reward, done, info]
        """
        time_steps = [env.step(action, self._blocking, i_env, train, scene_splits)
...
class ProcessPyEnvironment(object):
    ...
    def step(self, action, blocking, i_env, train, scene_splits):
        ...
        """
        promise = self.call('step', action, i_env, train, scene_splits)
        if blocking:
            return promise()
        else:
            return promise
        ...
        try:
            ...
            while True:
                ...
                if message == self._CALL:
                    ...
                    if name == 'step' or name == 'reset': #or name=='reload_model'
