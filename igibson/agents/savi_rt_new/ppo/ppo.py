...
class PPO(nn.Module):
    def __init__(
        ...
        for name, param in actor_critic.named_parameters():
            if name.startswith("net.visual_encoder.cnn") or \
                name.startswith("net.goal_encoder") or \
                name.startswith("net.action_encoder"):
                print(name, "has low lr!!!!!!", 0.1 * lr)
                param_groups.append({"params": param,'lr': 0.1 * lr})
            else:
                print(name, "has normal lr!!!!!!")
                param_groups.append({"params": param,'lr': lr})

        self.optimizer = optim.Adam(param_groups, eps=eps)
    ...
    def update(self, rollouts, loss_weight=None):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                # origin rt map shape (batch, 100, 100)
                rt_map = rt_map.view(rt_map.shape[0], 100, 100)
                # ground truth of rt map (batch, 100, 100)
                rt_map_gt = to_tensor(obs_batch['rt_map_gt']).to(self.device)

                # pass rt map and gt to the BCE loss
                rt_loss = self.loss(rt_map, rt_map_gt)
                ...
                # print(rt_num_correct)
                # accumulate the total pixels
                rt_num_sample += rt_map_gt.shape[0] * 100 ** 2
