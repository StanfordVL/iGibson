...
class SMTStateEncoder(nn.Module):
    ...
    def single_forward(self, x, memory, memory_masks, goal=None):
        ...
        if goal is not None:
            ...
            rt_output = rt_output.view(rt_output.shape[0], 1, 16, 16)

            rt_output = self.outc(rt_output, (rt_output.shape[0], self.rooms, self.rt_map_output_size, self.rt_map_output_size))
...
class outconv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.upconv = nn.ConvTranspose2d(in_ch, out_ch, 3, stride=2, padding=1)#nn.Conv2d(in_ch, out_ch, 1)

    def forward(self, x, out_size):
        x = self.upconv(x, output_size=out_size)
        return x