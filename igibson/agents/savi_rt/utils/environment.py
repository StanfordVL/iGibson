...
class AVNavRLEnv(iGibsonEnv):
    ...
    def __init__(self, config_file, mode, scene_id='mJXqzFtmKg4'):
        ...
        self.audio_len = 4410 # 44100 * action_time
        self.time_len = 10
...
#             orn_eframe = rpy[2] - initial_rpy[2]
            
#             state['pose_sensor'] = np.array(
#                 [*pos_eframe, orn_eframe, self._episode_time],
#                 dtype=np.float32)
            state['pose_sensor'] = np.array(
    ...
    def step(self, action, i_env, train):
        ...
        if done and self.automatic_reset:
            ...
            if (train and COUNT_CURR_EPISODE == self.config['TRAIN_EPISODE_COUNT_PER_SCENE']) or (not train and COUNT_CURR_EPISODE == self.config['TEST_EPISODE_COUNT_PER_SCENE']):
                if train:
