...
def main():
    ...
    scene_files = list(glob.glob(os.path.join(igibson.ig_dataset_path, "scenes", "Rs_int", "urdf/Rs_int.urdf"), recursive=True))
    by_scene = {}
    by_object = {x: [] for x in objects}
    for sf in scene_files:
        tree = ET.parse(sf)
        links = tree.findall(".//link[@category='table']")
        print(links[0].attrib["name"])
#         xyz
        ...
        for pair in objects:
            nodes = tree.findall(".//link[@category='%s']" % pair[0])
            if nodes:
