...
class ParallelNavEnv(iGibsonEnv):
    ...
    def reset(self, train=True):
        ...
        """
        time_steps = [env.reset(self._blocking, train) for env in self._envs]
        if not self._blocking:
            time_steps = [promise() for promise in time_steps]
        return time_steps

    def step(self, actions, train):
        """Forward a batch of actions to the wrapped environments.

        :param actions: batched action, possibly nested, to apply to the environment.
        :return: a list of [next_obs, reward, done, info]
        """
        time_steps = [env.step(action, self._blocking, i_env, train)
                      for i_env, (env, action) in enumerate(zip(self._envs, actions))]
...
class ProcessPyEnvironment(object):
    ...
    def start(self):
        ...
        self._process = multiprocessing.Process(target=self._worker,
                                                args=(conn, self._env_constructor, self._flatten), daemon=True)
    ...
    def step(self, action, blocking, i_env, train):
        ...
        """
        promise = self.call('step', action, i_env, train)
        if blocking:
            return promise()
        else:
            return promise

    def reset(self, blocking=True, train=True):
        """Reset the environment.

        :param blocking: whether to wait for the result.
        :return: next_obs when blocking, otherwise callable that returns next_obs
        """
        promise = self.call('reset', train)
        if blocking:
            return promise()
        else:
            return promise
