...
from igibson.render.mesh_renderer.materials import (
    RandomizedMaterial,
    ProceduralMaterial,
)
...
from igibson.utils.urdf_utils import (
...
try:
    ...
    print(
        "TaskNet could not be imported - object taxonomy / abilities will be unavailable.",
        file=sys.stderr,
    )
...
class ArticulatedObject(StatefulObject):
    ...
    def force_wakeup(self):
        ...
        for joint_id in range(p.getNumJoints(self.body_id)):
            p.changeDynamics(
                self.body_id, joint_id, activationState=p.ACTIVATION_STATE_WAKE_UP
            )
...
class URDFObject(StatefulObject):
    ...
    def __init__(
        ...
        if abilities is None:
            if OBJECT_TAXONOMY is not None:
                taxonomy_class = OBJECT_TAXONOMY.get_class_name_from_igibson_category(
                    self.category
                )
        ...
        assert isinstance(
            abilities, dict
        ), "Object abilities must be in dictionary form."
        ...
        if os.path.isfile(meta_json):
            with open(meta_json, "r") as f:
                ...
                if (
                    "orientations" in self.metadata
                    and len(self.metadata["orientations"]) > 0
                ):
    ...
    def compute_object_pose(self):
        if self.connecting_joint is not None:
            ...
            joint_xyz = np.array(
                [
                    float(val)
                    for val in self.connecting_joint.find("origin")
                    .attrib["xyz"]
                    .split(" ")
                ]
            )
    ...
    def load_supporting_surfaces(self):
        ...
        for predicate in heights:
            height_maps_dir = os.path.join(
                self.model_path, "misc", "height_maps_per_link", "{}".format(predicate)
            )
            ...
            for link_name in heights[predicate]:
                ...
                col_mesh_path = os.path.join(
                    self.model_path, link_col_mesh.attrib["filename"]
                )
        ...
        for joint_emb in self.object_tree.iter("joint"):
            ...
            for child_emb in joint_emb.findall("child"):
                ...
                if child_emb.attrib["link"] == "world":
                    ...
                    child_emb.attrib["link"] = (
                        self.name + "_" + child_emb.attrib["link"]
                    )
            ...
            for parent_emb in joint_emb.findall("parent"):
                ...
                if parent_emb.attrib["link"] == "world":
                    ...
                    parent_emb.attrib["link"] = (
                        self.name + "_" + parent_emb.attrib["link"]
                    )
    ...
    def scale_object(self):
        ...
        while not all_processed:
            ...
            for joint in self.object_tree.iter("joint"):
                ...
                if (
                    parent_link_name in scales_in_lf
                    and child_link_name not in scales_in_lf
                ):
        ...
        if self.overwrite_inertial and self.category not in [
            "walls",
            "floors",
            "ceilings",
        ]:
    ...
    def remove_floating_joints(self, folder=None):
        ...
        urdfs_no_floating = save_urdfs_without_floating_joints(
            self.object_tree, self.main_body_is_fixed, file_prefix
        )
    ...
    def prepare_randomized_texture(self):
        ...
        for i, urdf_path in enumerate(self.urdf_paths):
            ...
            for visual_mesh_path in visual_mesh_to_idx:
                ...
                if (
                    sub_urdf_tree.find(
                        ".//mesh[@filename='{}']".format(visual_mesh_path)
                    )
                    is not None
                ):
    ...
    def _load(self):
        ...
        for idx in range(len(self.urdf_paths)):
            ...
            body_id = p.loadURDF(
                self.urdf_paths[idx], flags=flags, useFixedBase=is_fixed
            )
    ...
    def force_wakeup(self):
        ...
        for body_id in self.body_ids:
            for joint_id in range(p.getNumJoints(body_id)):
                p.changeDynamics(
                    body_id, joint_id, activationState=p.ACTIVATION_STATE_WAKE_UP
                )
