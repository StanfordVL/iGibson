...
class LocomotorRobot(BaseRobot):
    ...
    def apply_robot_action(self, action):
        ...
        if self.control == "torque":
            ...
            if not hasattr(self, "wheel_axle_half") or not hasattr(
                self, "wheel_radius"
            ):
                raise Exception(
            ...
            left_wheel_ang_vel = (
                lin_vel - ang_vel * self.wheel_axle_half
            ) / self.wheel_radius
            right_wheel_ang_vel = (
                lin_vel + ang_vel * self.wheel_axle_half
            ) / self.wheel_radius
    ...
    def calc_state(self):
        ...
        """
        j = (
            np.array([j.get_joint_relative_state() for j in self.ordered_joints])
            .astype(np.float32)
            .flatten()
        )
