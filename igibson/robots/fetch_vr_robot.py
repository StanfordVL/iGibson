...
class FetchVR(Fetch):
    ...
    def __init__(
        ...
        control_hand="right",
        use_ns_ik=True,
        use_gaze_marker=True,
    ):
        config = parse_config(
            os.path.join(assets_path, "models", "fetch", "fetcH_embodiment.yaml")
        )
        ...
        LocomotorRobot.__init__(
            ...
            control=["differential_drive", "differential_drive"]
            + ["position"] * (self.torso_lift_dim + self.arm_dim),
            self_collision=True,
        )
        ...
        self.sim.import_object(
            self.effector_marker,
            use_pbr=False,
        ...
        self.gripper_ids = joints_from_names(
    ...
    def calc_ik_params(self):
        max_limits = (
            [0.0, 0.0] + get_max_limits(self.robot_id, self.arm_joint_ids) + [0.0, 0.0]
        )
        min_limits = (
            [0.0, 0.0] + get_min_limits(self.robot_id, self.arm_joint_ids) + [0.0, 0.0]
        )
        rest_position = (
            [0.0, 0.0]
    ...
    def update(self, vr_data=None):
        ...
        if is_valid:
            ...
            if self.use_ik_control and self.frame_count % self.update_freq == 0:
                ...
                self.apply_frame_data(
                    lin_vel, ang_vel, list(self.get_joint_pos()), grip_frac
                )
