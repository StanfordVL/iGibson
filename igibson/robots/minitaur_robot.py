...
class MinitaurBase(LocomotorRobot):
    ...
    def GetObservationUpperBound(self):
        ...
        upper_bound[
            self.num_motors : 2 * self.num_motors
        ] = motor.MOTOR_SPEED_LIMIT  # Joint velocity.
        upper_bound[
            2 * self.num_motors : 3 * self.num_motors
    ...
    def apply_action(self, motor_commands):
        ...
        if self.accurate_motor_model_enabled or self.pd_control_enabled:
            ...
            if self.accurate_motor_model_enabled:
                ...
                # Transform into the motor space when applying the torque.
                self.applied_motor_torques = np.multiply(
                    self.observed_motor_torques, self.motor_direction
                )

                for motor_id, motor_torque in zip(
...
class Minitaur(MinitaurBase):
    ...
    def __init__(
        self, config, env, pd_control_enabled=True, accurate_motor_model_enabled=True
    ):
