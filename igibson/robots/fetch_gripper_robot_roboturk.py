...
from igibson.external.pybullet_tools.utils import joints_from_names, set_joint_positions, joint_from_name,\
...
from igibson.controllers.ik_controller_roboturk import IKController
...
class FetchGripper(LocomotorRobot):
    def __init__(self, env, config):
        ...
        LocomotorRobot.__init__(self,
                                "fetch/fetch_gripper_roboturk{}.urdf".format("_vis" if self.gripper_visualization else ""),
    ...
    def calculate_head_wheel_joint_velocities(self, wheel_action):
        ...
        if self.tucked:
            ...
            ee_camera_coords = np.concatenate((self.env.renderer.transform_point(ee_pos), [1]))
            ee_pixel_coords = self.env.renderer.P.dot(ee_camera_coords)
