...
class BaseRobot(object):
    ...
    def __init__(self, model_file, base_name=None, scale=1, self_collision=False):
        ...
        if self.model_file[-4:] == "urdf":
            ...
            assert (
                self.scale == 1
            ), "pybullet does not support scaling for MJCF model (p.loadMJCF)"
        self.config = None
    ...
    def load(self):
        ...
        if self.model_type == "MJCF":
            self.robot_ids = p.loadMJCF(
                os.path.join(self.physics_model_dir, self.model_file), flags=flags
            )
        if self.model_type == "URDF":
            self.robot_ids = (
                p.loadURDF(
                    os.path.join(self.physics_model_dir, self.model_file),
        ...
        (
            ...
            self.robot_body,
            self.robot_mass,
        ) = self.parse_robot(self.robot_ids)
...
class BodyPart:
    ...
    def get_pose(self):
        """Get pose of body part"""
        return self._state_fields_of_pose_of(
            self.bodies[self.body_index], self.body_part_index
        )
    ...
    def contact_list(self):
        ...
        return p.getContactPoints(
            self.bodies[self.body_index], -1, self.body_part_index, -1
        )
