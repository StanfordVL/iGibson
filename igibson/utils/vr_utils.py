...
class VrData(object):
    ...
    def query(self, q):
        ...
        if q not in self.vr_data_dict.keys():
            raise RuntimeError(
                "ERROR: Key {} does not exist in VR data dictionary!".format(q)
            )
    ...
    def refresh_action_replay_data(self, ar_data, frame_num):
        ...
        for device in VR_DEVICES:
            device_data = ar_data["vr/vr_device_data/{}".format(device)][
                frame_num
            ].tolist()
            ...
            if device in VR_CONTROLLERS:
                ...
                if len(device_data) > 18:
                    self.vr_data_dict["{}_model_rotation".format(device)] = device_data[
                        17:21
                    ]
                self.vr_data_dict["{}_button".format(device)] = ar_data[
                    "vr/vr_button_data/{}".format(device)
                ][frame_num].tolist()

        torso_tracker_data = ar_data["vr/vr_device_data/torso_tracker"][
            frame_num
        ].tolist()
        self.vr_data_dict["torso_tracker"] = [
            torso_tracker_data[0],
            torso_tracker_data[1:4],
            torso_tracker_data[4:],
        ]

        eye_data = ar_data["vr/vr_eye_tracking_data"][frame_num].tolist()
        self.vr_data_dict["eye_data"] = [
            eye_data[0],
            eye_data[1:4],
            eye_data[4:7],
            eye_data[7],
...
def calc_offset(s, touch_x, touch_y, movement_speed, relative_device):
    ...
    return translate_vr_position_by_vecs(
        touch_x, touch_y, right, forward, curr_offset, movement_speed
    )
...
def get_normalized_translation_vec(right_frac, forward_frac, right, forward):
    ...
    vr_offset_vec = [
        right[i] * right_frac + forward[i] * forward_frac for i in range(3)
    ]
    vr_offset_vec[2] = 0
    return normalizeListVec(vr_offset_vec)


def translate_vr_position_by_vecs(
    right_frac, forward_frac, right, forward, curr_offset, movement_speed
):
    ...
    vr_offset_vec = get_normalized_translation_vec(
        right_frac, forward_frac, right, forward
    )
