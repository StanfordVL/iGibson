from material_util import (
    clean_nodes,
    build_pbr_textured_nodes_from_name,
    create_empty_image,
)
...
class redirect_output:
    def __init__(self):
        logfile = os.path.join(
            tempfile.gettempdir(), "blender_command_{}.log".format(getpass.getuser())
        )
...
def bake_model(
    ...
    if c in channels:
        ...
        if overwrite or not os.path.isfile("{}/{}.png".format(mat_dir, c)):
            ...
            for mat in bpy.data.materials:
                node = create_empty_image(
                    mat.node_tree, c, False, dim=(res, res), add_uv_node=add_uv_node
                )
                if node is not None:
                    node.select = True
    ...
    if c in channels:
        ...
        if overwrite or not os.path.isfile("{}/{}.png".format(mat_dir, c)):
            ...
            with redirect_output():
                bpy.ops.object.bake(
                    type=c, pass_filter={"DIRECT", "INDIRECT", "COLOR"}, margin=0
                )
            bpy.data.images[c].filepath_raw = "{}/{}.png".format(mat_dir, c)
            bpy.data.images[c].file_format = "PNG"
            bpy.data.images[c].save()
        ...
        if overwrite or not os.path.isfile("{}/{}.png".format(mat_dir, c)):
            ...
            for mat in bpy.data.materials:
                if (
                    ...
                    or "Principled BSDF" not in mat.node_tree.nodes
                ):
                    print("{} skipped".format(mat))
                    continue
    ...
    if c in channels:
        ...
        if overwrite or not os.path.isfile("{}/{}.png".format(mat_dir, c)):
            ...
            for mat in bpy.data.materials:
                node = create_empty_image(
                    mat.node_tree, c, True, dim=(res, res), add_uv_node=add_uv_node
                )
                if node is not None:
                    node.select = True
                    mat.node_tree.nodes.active = node
    ...
    if c in channels:
        ...
        if overwrite or not os.path.isfile("{}/{}.png".format(mat_dir, c)):
            ...
            with redirect_output():
                bpy.ops.object.bake(
                    type="DIFFUSE", pass_filter={"COLOR"}, margin=margin
                )
            bpy.data.images[c].filepath_raw = "{}/{}.png".format(mat_dir, c)
            bpy.data.images[c].file_format = "PNG"
            bpy.data.images[c].save()
...
def import_obj_folder(
    object_name, obj_dir, up="Z", forward="X", use_split_objects=False
):
...
def import_ig_object(
    ...
    if not import_mat:
        return
    mat = build_pbr_textured_nodes_from_name(
        "obj_mat", get_ig_texture_paths(object_root), is_cc0=False
    )
...
def export_obj_folder(
    save_dir, skip_empty=True, save_material=False, up="Z", forward="X"
):
    with redirect_output():
        for on in bpy.context.scene.objects.keys():
            obj = bpy.context.scene.objects[on]
...
def get_ig_scene_texture_paths(scene_root, element_name):
    def get_file_path(type):
        files = glob.glob(
            glob.escape(scene_root) + "/material/{}/*_{}.*".format(element_name, type)
        )
