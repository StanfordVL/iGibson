...
class Wall_bbox(object):
    def __init__(self, bbox, ceiling_height=2.4):
        ...
        self.rot_2d = np.array(
            [[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]]
        )

        self.rot_3d = np.array(
            [
    ...
    def add_hole(self, bbox):
        ...
        if l < self.l or r > self.r:
            ...
            # center_new = (l+r) / 2.
            if self.x_principle:
                straight_pts[:, 1] = np.clip(
                    straight_pts[:, 1], self.l + 0.01, self.r - 0.01
                )
            else:
                straight_pts[:, 0] = np.clip(
                    straight_pts[:, 0], self.l + 0.01, self.r - 0.01
                )
    ...
    def get_plane(self, front=True):
        ...
        if not self.x_principle:
            vertices = np.vstack(
                [vertices[:, 1], vertices_z, vertices[:, 0]]
            ).transpose()
            f_normal = self.vns["y_minus"] if front else self.vns["y_plus"]
        else:
            vertices = np.vstack(
                [vertices_z, vertices[:, 1], vertices[:, 0]]
            ).transpose()
    ...
    def add_skirt(self, skirt_pts):
                ...
                for x, z in skirt_pts:
                    skirt_verts.append(
                        ...
                        if self.x_principle
                            ...
                            seg_start + reverse_factor * x,
                            x_loc + reverse_factor * x,
                            z,
                        )
                    )
                for x, z in skirt_pts:
        ...
        return np.array(skirt_verts).dot(np.linalg.inv(self.rot_3d)), np.array(
            skirt_faces
        )
