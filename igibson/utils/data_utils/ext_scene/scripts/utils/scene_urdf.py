...
def get_component_string(obj_name, viz_mesh, collision_mesh, shift=(0.0, 0.0, 0.0), mass="5000"):
...
def gen_scene_urdf(model_dir, model_name, component=None):
    ...
    for m in meshes:
        ...
        if component is not None and component not in m:
            continue
        components += viz_mesh.format(obj_name=os.path.splitext(m)[0], viz_mesh="shape/visual/{}".format(m))
    ...
    for m in meshes:
        ...
        if component is not None and component not in m:
            continue
        components += col_mesh.format(collision_mesh="shape/collision/{}".format(m))
...
def gen_orig_urdf_with_cabinet(model_name, save_name):
    return orig_urdf_with_cabinet.format(model_name=model_name, save_name=save_name)
