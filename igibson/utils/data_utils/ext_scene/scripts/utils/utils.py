...
def semmap_to_lightmap(sem):
    ...
    kitchen_bathroom = np.logical_or(
        np.logical_or(sem_map == 10, sem_map == 1), sem_map == 19
    )
    kitchen_bathroom_filtered = cv2.dilate(
        kitchen_bathroom.astype(np.float32), kernel.astype(np.float32)
    )
...
class BBox(object):
    ...
    def get_scale(self):
        return (
            np.linalg.norm(self.edge_x),
            np.linalg.norm(self.edge_y),
            self.z[1] - self.z[0],
        )
