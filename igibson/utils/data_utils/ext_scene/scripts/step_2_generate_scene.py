        ...
        if sum(overlaps) > 1:
            ...
            for wi, o in enumerate(overlaps):
                if wi == max_idx:
                    ...
                    snap_out_of_wall(
                        hole_bbox[hi][0], wall_bbox[wi], hole, walls_poly[wi]
                    )
        elif sum(overlaps) == 1:
...
def main():
    ...
    if not os.path.isdir(args.model_dir):
        raise ValueError(
            "iGibson model directory does not exist: {}".format(args.model_dir)
        )
    ...
    for component in ["wall", "ceiling", "floor"]:
        with open(
            os.path.join(urdf_dir, "{}_{}s.urdf".format(model_name, component)), "w"
        ) as fp:
