...
parser.add_argument("--save_root", dest="save_root", default=os.path.join(igibson.cubicasa_dataset_path, "scenes"))
    ...
    for e in svg.getElementsByTagName("g"):
        if "FixedFurniture " in e.getAttribute("class"):
            ...
            if len(X) != 4:
                continue
            z_dim = get_z_dim(e)
            z_min = (z_dim["Elevation"]) / 100.0
            z_max = z_min + z_dim["Height"] / 100.0
...
def get_floor(svg):
    ...
    for e in svg.getElementsByTagName("g"):
        if "Space " in e.getAttribute("class"):
            ...
            X, Y = get_points(e)
            floor_polygons.append((room_type_ig, np.vstack([Y / 100.0, X / 100.0]).transpose()))
...
def main():
    ...
    for floor_i, mask_i in enumerate(masks):
        ...
        for bbox in wall_bboxes:
            y, x = (bbox.center * 100.0).astype(int)
            if y >= height:
                y = height - 1
            if x >= width:
                x = width - 1
        ...
        for poly in floor_polygons:
            y, x = (np.array(shape_poly(poly[1]).representative_point()) * 100.0).astype(int)
            if y >= height:
                y = height - 1
            if x >= width:
                x = width - 1
        ...
        if len(wall_bboxes_i) < 4 or len(floor_polygons_i) < 1:
            ...
            continue
        model_dir = os.path.join(args.save_root, "{}_floor_{}".format(model_id, floor_i))
        ...
        with open(os.path.join(save_dir, "furniture.json"), "w") as fp:
            json.dump([(cat, bbox.as_dict()) for cat, bbox in furniture_bboxes_i], fp)
        with open(os.path.join(save_dir, "floor.json"), "w") as fp:
            json.dump([(cat, poly.tolist()) for cat, poly in floor_polygons_i], fp)
        ...
        for i, (cat, poly) in enumerate(floor_polygons_i):
            ...
            pts = [((x + max_length) * 100.0, (y + max_length) * 100.0) for x, y in poly]
        ...
        if args.viz:
            ...
            wall_patches = [Polygon(bbox.get_coords(), True, color="b") for bbox in wall_bboxes_i]
            window_patches = [Polygon(bbox.get_coords(), True, color="b") for bbox in window_bboxes_i]
            door_patches = [Polygon(bbox.get_coords(), True, color="b") for bbox in door_bboxes_i]
            furn_patches = [Polygon(bbox.get_coords(), True, color="b") for c, bbox in furniture_bboxes_i]
            floor_patches = [Polygon(poly, True, color="b") for cat, poly in floor_polygons_i]

            quivers = [np.array([*bbox.center, *bbox.nrm]) for c, bbox in furniture_bboxes_i]
            ...
            ax.set_ylim([0, width / 100.0])
            ax.set_xlim([0, height / 100.0])
            if len(quivers) > 0:
