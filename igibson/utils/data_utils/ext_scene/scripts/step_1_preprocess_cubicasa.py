...
def get_z_dim(e):
    ...
    for d in dim_infos:
        if ":" not in d or len(d.split(":")) != 2:
            raise ValueError(
                "Furniture dim should have : as separator, and two elements. {}".format(
                    dim_infos
                )
            )
        k, v = d.split(":")
    ...
    for e in svg.getElementsByTagName("g"):
        if "FixedFurniture " in e.getAttribute("class"):
            ...
            bbox = polygon_to_bbox(
                Y, X, (z_min, z_max), get_rotation_angle(e), flip_image=wall_image
            )
    ...
    for e in svg.getElementsByTagName("g"):
        if "Space " in e.getAttribute("class"):
            ...
            floor_polygons.append(
                (room_type_ig, np.vstack([Y / 100.0, X / 100.0]).transpose())
            )
...
def main():
    ...
    for floor_i, mask_i in enumerate(masks):
        ...
        for poly in floor_polygons:
            y, x = (
                np.array(shape_poly(poly[1]).representative_point()) * 100.0
            ).astype(int)
            if y >= height:
                y = height - 1
            if x >= width:
        ...
        model_dir = os.path.join(
            args.save_root, "{}_floor_{}".format(model_id, floor_i)
        )
        ...
        for i, (cat, poly) in enumerate(floor_polygons_i):
            ...
            pts = [
                ((x + max_length) * 100.0, (y + max_length) * 100.0) for x, y in poly
            ]
        ...
        padded_image.paste(
            sem_image, ((3000 - og_size[0]) // 2, (3000 - og_size[1]) // 2)
        )
        ...
        if args.viz:
            ...
            wall_patches = [
                Polygon(bbox.get_coords(), True, color="b") for bbox in wall_bboxes_i
            ]
            window_patches = [
                Polygon(bbox.get_coords(), True, color="b") for bbox in window_bboxes_i
            ]
            door_patches = [
                Polygon(bbox.get_coords(), True, color="b") for bbox in door_bboxes_i
            ]
            furn_patches = [
            ...
            floor_patches = [
                Polygon(poly, True, color="b") for cat, poly in floor_polygons_i
            ]

            quivers = [
                np.array([*bbox.center, *bbox.nrm]) for c, bbox in furniture_bboxes_i
            ]
