...
def process_scene_wall(model_id):
    ...
    polys = [
        mesh_to_xy_convex_hull(np.array(w["xyz"]).reshape([-1, 3]))
        for w in wall_components
    ]
    rects = [minimum_bounding_rectangle(p) for p in polys]
    bboxes = [
        polygon_to_bbox(p[:, 0], p[:, 1], (0.0, z_max), scale_factor=1.0) for p in rects
    ]
    return polys, bboxes
...
def process_hole_elements(model_id, cat):
    ...
    with open(model_id, "r") as fp:
        data = json.load(fp)
    components = [
        f for f in data["mesh"] if f["type"] == cat and get_bbox(f)[1][1] > 0.05
    ]
    polys = [
        mesh_to_xy_convex_hull(np.array(w["xyz"]).reshape([-1, 3])) for w in components
    ]
    zs = []
    for w in components:
    ...
    rects = [minimum_bounding_rectangle(p) for p in polys]
    bboxes = [
        polygon_to_bbox(p[:, 0], p[:, 1], z, scale_factor=1.0)
        for p, z in zip(rects, zs)
        if z[0] != z[1]
    ]
    return polys, bboxes
...
def process_scene_rooms(model_id):
    ...
    platform_polys = [
        mesh_to_xy_convex_hull(np.array(w["xyz"]).reshape([-1, 3])) for w in platforms
    ]
    to_fill = []
    if len(platforms) == 0:
        return to_fill
    ...
    xmin, _, ymin = smin
    xmax, _, ymax = smax
    height = int(100.0 * (xmax - xmin)) + 20
    width = int(100.0 * (ymax - ymin)) + 20
    floor_img = Image.new("1", (height, width), 0)
    d = ImageDraw.Draw(floor_img)
    for coords in floor_polys:
        x = (coords[:, 0] - xmin) * 100.0
        y = (coords[:, 1] - ymin) * 100.0
...
def gen_static_cabinet_info(save_dir, save_name):
    ...
    vm_path = os.path.join(save_dir, "shape", "visual", "{}_vm.obj".format(save_name))
    cm_path = os.path.join(
        save_dir, "shape", "collision", "{}_cm.obj".format(save_name)
    )
    cmd = "../../blender_utils/vhacd --input {} --output {}".format(vm_path, cm_path)
    ...
    if not os.path.isfile:
        ...
        subprocess.call(cmd, shell=True, stdout=subprocess.DEVNULL)
    with open(
        os.path.join(urdf_dir, "{}_{}s.urdf".format(model_name, save_name)), "w"
    ) as fp:
...
def gen_room_maps(model_id, viz=False):
    ...
    for i, r in enumerate(room):
        ...
        hull_pts = (mesh_to_xy_convex_hull(verts) + max_length) * 100.0
    ...
    padded_image.paste(sem_image, ((3000 - og_size[0]) // 2, (3000 - og_size[1]) // 2))
...
def main():
    ...
    if should_skip(json_path):
        quit()
    model_id = os.path.splitext(os.path.basename(os.path.normpath(json_path)))[0]
    ...
    with open(os.path.join(misc_dir, "window.json"), "w") as fp:
        json.dump(
            [bbox.as_dict() for bbox in process_hole_elements(json_path, "Window")[1]],
            fp,
        )
    with open(os.path.join(misc_dir, "door.json"), "w") as fp:
        json.dump(
            [bbox.as_dict() for bbox in process_hole_elements(json_path, "Door")[1]], fp
        )
    with open(os.path.join(misc_dir, "hole.json"), "w") as fp:
        json.dump(
            [bbox.as_dict() for bbox in process_hole_elements(json_path, "Hole")[1]], fp
        )
    with open(os.path.join(misc_dir, "floor.json"), "w") as fp:
        json.dump(
            [("undefined", poly.tolist()) for poly in process_scene_rooms(json_path)],
            fp,
        )
