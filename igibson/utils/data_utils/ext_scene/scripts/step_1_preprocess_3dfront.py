...
parser.add_argument("--save_root", dest="save_root", default=os.path.join(igibson.threedfront_dataset_path, "scenes"))
...
def process_scene_wall(model_id):
    ...
    bboxes = [polygon_to_bbox(p[:, 0], p[:, 1], (0.0, z_max), scale_factor=1.0) for p in rects]
    return polys, bboxes
...
def process_hole_elements(model_id, cat):
    ...
    rects = [minimum_bounding_rectangle(p) for p in polys]
    bboxes = [polygon_to_bbox(p[:, 0], p[:, 1], z, scale_factor=1.0) for p, z in zip(rects, zs) if z[0] != z[1]]
    return polys, bboxes
...
def gen_static_cabinet_info(save_dir, save_name):
    ...
    cm_path = os.path.join(save_dir, "shape", "collision", "{}_cm.obj".format(save_name))
    ...
    if not os.path.isfile:
        ...
        subprocess.call(cmd, shell=True, stdout=subprocess.DEVNULL)
    with open(os.path.join(urdf_dir, "{}_{}s.urdf".format(model_name, save_name)), "w") as fp:
...
def get_all_objs(model_id):
    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "utils", "data", "model_ig_cat.json"), "r"
    ) as fp:
        model_to_cat = json.load(fp)
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "utils", "data", "model_bbox.json"), "r") as fp:
    ...
    for r in room:
        ...
        for c in children:
            ...
            if np.sum(axis) != 0 and not math.isnan(theta):
                ...
                R = np.array([[aa + bb - cc - dd, 2 * (bd - ac)], [2 * (bd + ac), aa + dd - bb - cc]])
            ...
            y = edge_x / 2.0
            x = edge_y / 2.0
            raw_pts = np.array([center - x - y, center + x - y, center + x + y, center - x + y])
