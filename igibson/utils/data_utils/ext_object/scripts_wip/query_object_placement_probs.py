...
def input_number_or_name(selection_name, options):
    ...
    user_input = input(
        "Input number or name to assign probability, or done to finish\n"
    )
    if user_input == "done":
        return "done"
    ...
    try:
        ...
        if user_input.lower() not in [option.lower() for option in options]:
            ...
            return False
        choice = [option for option in options if option.lower() == user_input.lower()][
            0
        ]
...
def main(args):
    ...
    for scene_name in scene_names:
        support_objs_json = (
            "data/ig_dataset/scenes/%s/misc/all_support_objs.json" % scene_name
        )
        if os.path.isfile(support_objs_json):
            ...
            settings = MeshRendererSettings(
                enable_shadow=False, msaa=False, enable_pbr=False
            )
            s = Simulator(
                mode="headless",
                image_width=800,
            ...
            for obj_name in scene.objects_by_name:
                ...
                info_dict["supporting_surface_types"] = list(
                    obj.supporting_surfaces.keys()
                )
    ...
    for support_obj_dict in support_obj_dicts:
        ...
        room_category_support_types[(obj_category, obj_room)] = support_obj_dict[
            "supporting_surface_types"
        ]
