    ...
    for scene_name in all_scene_names:
        ...
        for category, count in objects_to_sample:
            ...
            for i in range(count):
                ...
                urdf_path = "%s/%s/%s.urdf" % (
                    object_cat_dirs[category],
                    object_id,
                    object_id,
                )
                ...
                for attempt in range(args.num_attempts):
                    ...
                    placement_rules_path = os.path.join(
                        urdf_object.model_path, "misc", "placement_probs.json"
                    )
                    ...
                    for placement_rule in placement_rules.keys():
                        ...
                        if (support_obj_cat, room) in category_supporting_objects:
                            valid_placement_rules[placement_rule] = placement_rules[
                                placement_rule
                            ]
                    ...
                    placement_rule = random.choices(
                        list(valid_placement_rules.keys()),
                        weights=valid_placement_rules.values(),
                        k=1,
                    )[0]
                    support_obj_cat, room, predicate = placement_rule.split("-")
                    if predicate == "ontop":
                    ...
                    print(
                        "Sampling %s %s %s %s in %s"
                        % (category, object_id, predicate, support_obj_cat, room)
                    )
                    result = sample_kinematics(
                        predicate, urdf_object, chosen_support_obj, True
                    )
                    if not result:
                    ...
                    if args.save_images:
                        simulator.sync()
                        scene.open_one_obj(
                            chosen_support_obj.body_ids[chosen_support_obj.main_body],
                            "max",
                        )
                        ...
                        for i in range(4):
                            ...
                            plt.imshow(frame)
                            plt.savefig(
                                "placement_imgs/%s_placement_%d_%d.png"
                                % (scene_name, placement_count, i)
                            )
                            plt.close()
                        scene.open_one_obj(
                            chosen_support_obj.body_ids[chosen_support_obj.main_body],
        ...
        if args.save_placement_txt:
            with open("%s_placements.txt" % scene_name, "w") as f:
                for room in room_placements_counts:
                    ...
                    for support_cat in [
                        cat for cat, r in placements_counts if r == room
                    ]:
                        f.write(
                            "\t%s: %d\n"
