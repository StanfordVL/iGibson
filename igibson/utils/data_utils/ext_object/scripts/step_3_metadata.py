...
def gen_bbox(input_dir):
    ...
    save_dict = {
        "base_link_offset": tuple((max_c + min_c) / 2.0),
...
def gen_material(input_dir, material_string):
    ...
    assert os.path.isfile(material_json_file), "cannot find material files: {}".format(
        material_json_file
    )
...
def composite_transmission_material(input_dir):
    ...
    transmission_low_res = np.array(
        Image.open(transmissio_fn).resize(metallic.shape[:2])
    )
