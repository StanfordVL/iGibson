...
if import_axis_up not in axis:
    raise ValueError(
        "Axis up not supported: {} (should be among X,Y,Z,-X,-Y,-Z)".format(
            import_axis_up
        )
    )
...
if import_axis_forward not in axis:
    raise ValueError(
        "Axis forward not supported: {} (should be among X,Y,Z,-X,-Y,-Z)".format(
            import_axis_forward
        )
    )
...
if source_blend_file is not None:
    ...
    import_obj_folder(
        model_id, source_dir, up=import_axis_up, forward=import_axis_forward
    )
