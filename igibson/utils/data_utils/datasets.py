...
def get_item_fn(
    ...
    paths = [
        os.path.join(
            ...
            "pano",
            "rgb",
            "point_" + item + "_view_equirectangular_domain_rgb.png",
        )
        for item in uuids
    ]
    mist_paths = [
        os.path.join(
            ...
            "pano",
            "mist",
            "point_" + item + "_view_equirectangular_domain_mist.png",
        )
        for item in uuids
    ]
    normal_paths = [
        os.path.join(
            ...
            "pano",
            "normal",
            "point_" + item + "_view_equirectangular_domain_normal.png",
        )
        for item in uuids
    ]
    pose_paths = [
        os.path.join(root, scene, "pano", "points", "point_" + item + ".json")
        for item in uuids
    ]
    semantic_paths = [
        os.path.join(
    ...
    for i, item in enumerate(pose_paths):
        ...
        p = (
            np.concatenate(np.array(pose_dict[1][u"camera_rt_matrix"]))
            .astype(np.float32)
            .reshape((4, 4))
        )
    ...
    if not off_3d and require_rgb:
        mist_imgs = [
            np.expand_dims(np.array(item).astype(np.float32) / 65536.0, 2)
            for item in mist_imgs
        ]
        org_mist = mist_imgs[0][:, :, 0].copy()
        mist_target = np.expand_dims(
            np.array(mist_target).astype(np.float32) / 65536.0, 2
        )
...
""" class ViewDataSet3D(data.Dataset):
    def __init__(self,
        ...
        self._require_rgb = "RGB_FILLED" in self.env.config[
            "ui_components"] or "RGB_PREFILLED" in self.env.config[
                "ui_components"] or "rgb_filled" in self.env.config[
                    "output"] or "rgb_prefill" in self.env.config["output"]
        self.off_3d = off_3d
        self.select = []
        ...
        if overwrite_fofn or not os.path.isfile(self.fofn):
            ...
            if only_load is None:
                self.scenes = sorted([
                    ...
                    and os.path.isfile(os.path.join(self.root, d, 'camera_poses.csv'))
            ...
            for scene in self.scenes[:last]:
                ...
                with open(posefile) as f:
                    for line in f:
                        ...
                        # print(uuid, xyz)
                        if os.path.isfile(
                                os.path.join(self.root, scene, 'pano', 'points',
                                             'point_' + uuid + '.json')):
                            if np.linalg.norm(np.array(xyz) - np.array([0, 0, 0])
                                              ) > 1e-5:    #remove scans that are not registered
            ...
            for scene, meta in tqdm(list(self.meta.items())):
                ...
                for uuid, v in list(meta.items()):
                    dist_list = [(uuid2, np.linalg.norm(np.array(v2[1]) - np.array(v[1])))
                                 for uuid2, v2 in list(meta.items())]
                    dist_list = sorted(dist_list, key=lambda x: x[-1])

                    if not dist_filter is None:
                        if dist_list[1][-1] < dist_filter:
                            self.select.append([[scene, dist_list[i][0], dist_list[i][1]]
    ...
    def get_scene_info(self, index):
        ...
        pose_paths = ([
            os.path.join(self.root, scene, 'pano', 'points', "point_" + item[0] + ".json")
            for item in uuids
        ])
        poses = []
        for item in pose_paths:
            f = open(item)
            pose_dict = json.load(f)
            p = np.concatenate(np.array(pose_dict[1][u'camera_rt_matrix'])).astype(
    ...
    def __getitem__(self, index):
        ...
        paths = ([
            os.path.join(self.root, scene, 'pano', 'rgb',
                         "point_" + item + "_view_equirectangular_domain_rgb.png") for item in uuids
        ])
        mist_paths = ([
            os.path.join(self.root, scene, 'pano', 'mist',
                         "point_" + item + "_view_equirectangular_domain_mist.png")
            for item in uuids
        ])
        normal_paths = ([
            os.path.join(self.root, scene, 'pano', 'normal',
                         "point_" + item + "_view_equirectangular_domain_normal.png")
            for item in uuids
        ])
        pose_paths = ([
            os.path.join(self.root, scene, 'pano', 'points', "point_" + item + ".json")
            for item in uuids
        ])
        semantic_paths = ([
            os.path.join(self.root, scene, 'pano', 'semantic',
                         "point_" + item + "_view_equirectangular_domain_semantic.png")
            for item in uuids
        ])
        poses = []
        ...
        if not self.off_pc_render and self._require_rgb:
            ...
            depth = org_mist
            pose = poses_relative[0].numpy()
            self.dll.render(ct.c_int(img.shape[0]), ct.c_int(img.shape[1]),
                            img.ctypes.data_as(ct.c_void_p), depth.ctypes.data_as(ct.c_void_p),
                            pose.ctypes.data_as(ct.c_void_p), render.ctypes.data_as(ct.c_void_p),
                            target_depth.ctypes.data_as(ct.c_void_p))
            if not self.transform is None:
    ...
    def get_multi_index(self, uuids):
        ...
        p = Pool(16)
        partial_fn = partial(get_item_fn,
                             select=self.select,
                             root=self.root,
                             loader=self.loader,
                             ...
                             depth_trans=self.depth_trans,
                             off_pc_render=self.off_pc_render,
                             dll=self.dll,
                             train=self.train,
            ...
            for subdir, dirs, files in os.walk(self.root):
                ...
                for file in files:
                    if file[-3:] == 'png' and file[:3] == 'img':
                        self.fns.append(
                            (os.path.join(subdir, file), os.path.join(subdir, 'render' + file[3:])))
            with open(self.fofn, 'wb') as fp:
                pickle.dump(self.fns, fp)
        else:
            with open(self.fofn, 'rb') as fp:
...
if __name__ == '__main__':
    ...
    if opt.dataset == 'view3d':
        ...
        d = RenderPairDataset(root=opt.dataroot,
