...
def fit_plane(points):
    ...
    ), "Cannot fit plane with only {} points in {} dimensions.".format(
...
def get_parallel_rays(
    ...
    sources = [
        source + np.dot(offsets, orthogonal_vectors) for offsets in ray_grid_flattened
    ]
    destinations = [
        destination + np.dot(offsets, orthogonal_vectors)
        for offsets in ray_grid_flattened
    ]

    return sources, destinations, ray_grid


def sample_origin_positions(
    mins,
    maxes,
    count,
    ...
    for i in range(count):
        ...
        bimodal_sample = truncnorm.rvs(
            bottom, top, loc=bimodal_mean_fraction, scale=bimodal_stdev_fraction
        )
        ...
        position[bimodal_axis] = (
            bimodal_sample if bimodal_axis_top_side else 1 - bimodal_sample
        )
...
def sample_cuboid_on_object(
    ...
    assert (
        cuboid_dimensions.shape[-1] == 3
    ), "Cuboid dimensions need to contain all three dimensions."
    if cuboid_dimensions.ndim == 2:
        assert (
            cuboid_dimensions.shape[0] == num_samples
        ), "Need as many offsets as samples requested."
    ...
    for i in range(num_samples):
        ...
        for axis, is_top, start_pos in samples:
            ...
            cast_results = p.rayTestBatch(
                rayFromPositions=sources, rayToPositions=destinations, numThreads=0
            )

            # Check that all rays hit the object.
            if not check_rays_hit_object(
                cast_results, body_id, refusal_reasons["missed_object"]
            ):
                continue
            ...
            if not check_normal_similarity(
                center_hit_normal, hit_normals, refusal_reasons["hit_normal_similarity"]
            ):
                continue
            ...
            distances = get_distance_to_plane(
                hit_positions, plane_centroid, plane_normal
            )
            if np.any(distances > hit_to_plane_threshold):
                if igibson.debug_sampling:
                    refusal_reasons["dist_to_plane"].append(
                        "distances to plane: %r" % (distances,)
                    )
                continue

            # Get projection of the base onto the plane, fit a rotation, and compute the new center hit / corners.
            projected_hits = get_projection_onto_plane(
                hit_positions, plane_centroid, plane_normal
            )
            ...
            cuboid_centroid = (
                center_projected_hit + plane_normal * this_cuboid_dimensions[2] / 2.0
            )
...
def compute_rotation_from_grid_sample(
    two_d_grid, hit_positions, cuboid_centroid, this_cuboid_dimensions
):
    ...
    grid_in_object_coordinates /= np.linalg.norm(grid_in_object_coordinates, axis=1)[
        :, None
    ]

    sampled_grid_relative_vectors = hit_positions - cuboid_centroid
    sampled_grid_relative_vectors /= np.linalg.norm(
        sampled_grid_relative_vectors, axis=1
    )[:, None]
    ...
    rotation, _ = Rotation.align_vectors(
        sampled_grid_relative_vectors, grid_in_object_coordinates
    )
    return rotation
...
def check_normal_similarity(center_hit_normal, hit_normals, refusal_log):
    ...
    parallel_hit_normal_angles_to_hit_normal = np.arccos(
        parallel_hit_main_hit_dot_products
    )
...
def check_hit_max_angle_from_z_axis(hit_normal, max_angle_with_z_axis, refusal_log):
    hit_angle_with_z = np.arccos(
        np.clip(np.dot(hit_normal, np.array([0, 0, 1])), -1.0, 1.0)
    )
...
def compute_ray_destination(axis, is_top, start_pos, aabb_min, aabb_max):
    ...
    multiple_to_face = np.min(
        multiple_to_face_on_each_axis[np.isfinite(multiple_to_face_on_each_axis)]
    )
...
def check_cuboid_empty(
    hit_normal, bottom_corner_positions, refusal_log, this_cuboid_dimensions
):
    # Compute top corners.
    top_corner_positions = (
        bottom_corner_positions + hit_normal * this_cuboid_dimensions[2]
    )
    ...
    top_to_bottom_pairs = list(
        itertools.product(top_corner_positions, bottom_corner_positions)
    )
    ...
    check_cast_results = p.rayTestBatch(
        rayFromPositions=all_pairs[:, 0, :],
        rayToPositions=all_pairs[:, 1, :],
        numThreads=0,
    )
