...
def get_xy_floors(vertices, faces, dist_threshold=-0.98):
    ...
    for face in tqdm(faces):
        ...
        if (dist_threshold is None) or (
            (dist_threshold is not None) and (dist < dist_threshold)
        ):
...
def gen_trav_map(
    ...
    wall_maps = gen_map(
        vertices, faces, output_folder, img_filename_format=obstacle_map_filename_format
    )
    ...
    cv2.fillPoly(
        wall_map_hull,
        [
            wall_convex_hull.points[wall_convex_hull.vertices][:, ::-1]
            .reshape((-1, 1, 2))
            .astype(np.int32)
        ],
        255,
    ...
    for i_floor in range(len(floors)):
        ...
        cur_img.save(
            os.path.join(output_folder, trav_map_filename_format.format(i_floor))
        )
...
def compute_triangle_plane_intersections(
    vertices, faces, tid, plane, dists, dist_tol=1e-8
):
    """
