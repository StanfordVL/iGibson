...
class MotionPlanningWrapper(object):
    ...
    def plan_base_motion(self, goal):
        ...
        if self.collision_with_pb_2d_planning:
            ...
            obstacles = [item.get_body_ids()[0] for item in self.env.scene.objects_by_category["bottom_cabinet"]]
        else:
            obstacles = []
    ...
    def dry_run_arm_plan(self, arm_path, object_in_hand=False, object_id=None):
        """
    ...
    def interact_push(self, push_point, push_direction):
        """
    ...
    def execute_arm_push(self, plan, hit_pos, hit_normal):
        ...
        if plan is not None:
            ...
            self.interact_push(hit_pos, hit_normal)
