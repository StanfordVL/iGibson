...
from igibson.utils.vr_utils import (
    VrData,
    convert_button_data_to_binary,
    VR_BUTTON_COMBO_NUM,
)
...
class IGLogWriter(object):
    ...
    def generate_name_path_data(self):
        ...
        if self.task:
            self.name_path_data.extend(
                [["goal_status", "satisfied"], ["goal_status", "unsatisfied"]]
            )
        if self.store_vr:
            self.name_path_data.extend(
                [
    ...
    def create_data_map(self):
        ...
        if self.task:
            ...
            self.data_map["goal_status"] = {
                "satisfied": np.full(
                    (self.frames_before_write, self.total_goals),
                    self.default_fill_sentinel,
                ),
                "unsatisfied": np.full(
                    (self.frames_before_write, self.total_goals),
        ...
        for bid in self.tracked_objects:
            ...
            handle["joint_state"] = np.full(
                (self.frames_before_write, self.joint_map[bid]),
                self.default_fill_sentinel,
            )
        ...
        if self.store_vr:
            self.data_map["vr"] = {
                ...
                "vr_eye_tracking_data": np.full(
                    (self.frames_before_write, 9),
                    self.default_fill_sentinel,
                    dtype=self.np_dtype,
                ),
                "vr_event_data": {
                    "left_controller": np.full(
                        (self.frames_before_write, VR_BUTTON_COMBO_NUM),
                        self.default_fill_sentinel,
                        dtype=self.np_dtype,
                    ),
                    "right_controller": np.full(
    ...
    def register_action(self, action_path, action_shape):
        ...
        curr_dict[path_tokens[-1]] = np.full(
            extended_shape, self.default_fill_sentinel, dtype=self.np_dtype
        )
    ...
    def set_up_data_storage(self):
        ...
        for name_path in self.name_path_data:
            ...
            hf.create_dataset(
                joined_path, curr_data_shape, maxshape=max_shape, dtype=np.float64
            )
        ...
        if self.task:
            # TODO FIX ME FIX ME PLEASE DON'T FORGET MICHAEL 7_27
            ...
            self.hf.attrs[
                "/metadata/obj_body_id_to_name"
            ] = self.obj_body_id_to_name_str
        # VR config YML is stored as a string in metadata
        if self.store_vr:
            self.hf.attrs[
                "/metadata/vr_settings"
            ] = self.sim.vr_settings.dump_vr_settings()
    ...
    def write_vr_data_to_map(self):
        ...
        self.data_map["vr"]["vr_camera"]["right_eye_view"][
            self.frame_counter, ...
        ] = self.sim.renderer.V
        self.data_map["vr"]["vr_camera"]["right_eye_proj"][
            self.frame_counter, ...
        ] = self.sim.renderer.P
        self.data_map["vr"]["vr_camera"]["right_camera_pos"][
            self.frame_counter, ...
        ] = self.sim.renderer.camera

        if self.vr_robot:
            forces = {
                "left_controller": p.getConstraintState(
                    self.vr_robot.parts["left_hand"].movement_cid
                ),
                "right_controller": p.getConstraintState(
                    self.vr_robot.parts["right_hand"].movement_cid
                ),
            }
        for device in ["hmd", "left_controller", "right_controller"]:
            ...
            if device == "left_controller" or device == "right_controller":
                ...
                if button_data_list[0] is not None:
                    self.data_map["vr"]["vr_button_data"][device][
                        self.frame_counter, ...
                    ] = np.array(button_data_list)

        is_valid, torso_trans, torso_rot = self.sim.get_data_for_vr_tracker(
            self.sim.vr_settings.torso_tracker_serial
        )
        torso_data_list = [is_valid]
        torso_data_list.extend(list(torso_trans))
        torso_data_list.extend(list(torso_rot))
        self.data_map["vr"]["vr_device_data"]["torso_tracker"][
            self.frame_counter, ...
        ] = np.array(torso_data_list)
        ...
        if self.vr_robot:
            vr_pos_data.extend(
                p.getConstraintState(self.vr_robot.parts["body"].movement_cid)
            )
        self.data_map["vr"]["vr_device_data"]["vr_position_data"][
            self.frame_counter, ...
        ] = np.array(vr_pos_data)
        ...
        ) = self.sim.get_eye_tracking_data()
        if is_valid:
            ...
            self.data_map["vr"]["vr_eye_tracking_data"][
                self.frame_counter, ...
            ] = np.array(eye_data_list)
        ...
        for controller in controller_events.keys():
            bin_button_data = convert_button_data_to_binary(
                controller_events[controller]
            )
            self.data_map["vr"]["vr_event_data"][controller][
                self.frame_counter, ...
            ] = np.array(bin_button_data)

        reset_actions = []
        for controller in controller_events.keys():
            reset_actions.append(self.sim.query_vr_event(controller, "reset_agent"))
        self.data_map["vr"]["vr_event_data"]["reset_actions"][
            self.frame_counter, ...
        ] = np.array(reset_actions)
        ...
        if self.task and self.filter_objects:
            for obj_bid, obj in self.tracked_objects.items():
                ...
                handle["joint_state"][self.frame_counter] = np.array(
                    [
                        p.getJointState(obj_bid, n)[0]
                        for n in range(self.joint_map[obj_bid])
                    ]
                )
        else:
            for bid in self.tracked_objects:
    ...
    def _print_pybullet_data(self):
        ...
        print(
            "----- PyBullet data at the end of frame {} -----".format(
                self.persistent_frame_count
            )
        )
        if self.task and self.filter_objects:
    ...
    def write_predicate_data_to_map(self):
        ...
        self.data_map["goal_status"]["unsatisfied"][
            self.frame_counter
        ] = self.one_hot_encoding(unsatisfied, self.total_goals)
        self.data_map["goal_status"]["satisfied"][
            self.frame_counter
        ] = self.one_hot_encoding(satisfied, self.total_goals)

    def write_agent_data_to_map(self):
        self.data_map["agent_actions"]["vr_robot"][
            self.frame_counter
        ] = self.vr_robot.dump_action()
    ...
    def write_to_hd5(self):
        ...
        if self.log_status:
            print(
                "----- Writing log data to hd5 on frame: {0} -----".format(
                    self.persistent_frame_count
                )
            )
        ...
        for name_path in self.name_path_data:
            ...
            curr_dset[-self.frames_before_write :, ...] = self.get_data_for_name_path(
                name_path
            )
    ...
    def end_log_session(self):
        ...
        if self.log_status:
            print(
                "IG LOGGER INFO: Ending log writing session after {} frames".format(
                    self.persistent_frame_count
                )
            )
        self.hf.close()
...
class IGLogReader(object):
    ...
    def set_replay_camera(self, sim):
        ...
        sim.renderer.set_light_position_direction(
            [right_cam_pos[0], right_cam_pos[1], 10],
    ...
    def get_agent_action(self, agent_name):
        ...
        if agent_action_path not in self.hf:
            raise RuntimeError(
                "Unable to find agent action path: {} in saved HDF5 file".format(
                    agent_action_path
                )
            )
    ...
    def end_log_session(self):
        ...
        if self.log_status:
            print(
                "Ending frame reading session after reading {0} frames".format(
