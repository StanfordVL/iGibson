...
def get_ig_scene_path(scene_name):
    ...
    assert scene_name in os.listdir(ig_scenes_path), "Scene {} does not exist".format(
        scene_name
    )
...
def get_3dfront_scene_path(scene_name):
    ...
    assert scene_name in os.listdir(
        threedfront_dataset_path
    ), "Scene {} does not exist".format(scene_name)
...
def get_ig_category_path(category_name):
    ...
    assert category_name in os.listdir(
        ig_categories_path
    ), "Category {} does not exist".format(category_name)
...
def get_ig_model_path(category_name, model_name):
    ...
    assert model_name in os.listdir(
        ig_category_path
    ), "Model {} from category {} does not exist".format(model_name, category_name)
...
def get_all_object_models():
    ...
    categories = [
        ...
        if os.path.isdir(os.path.join(ig_categories_path, item))
    ]
    models = []
    for category in categories:
        ...
        category_models = [
            ...
            if os.path.isdir(os.path.join(ig_categories_path, category, item))
        ]
        models.extend(
            [
                os.path.join(ig_categories_path, category, item)
                for item in category_models
            ]
        )
    return models
...
def get_ig_assets_version():
    ...
    process = subprocess.Popen(
        ["git", "-C", igibson.ig_dataset_path, "rev-parse", "HEAD"],
        shell=False,
...
def get_scene_path(scene_id):
    ...
    assert (
        scene_id in os.listdir(data_path) or scene_id == "stadium"
    ), "Scene {} does not exist".format(scene_id)
...
def download_assets():
    ...
    if not os.path.exists(igibson.assets_path):
        ...
        os.system(
            "tar -zxf /tmp/assets_igibson.tar.gz --directory {}".format(
...
def download_dataset(url):
    ...
    os.system(
        "wget -c --retry-connrefused --tries=5 --timeout=5 {} -O /tmp/{}".format(
            url, file_name
        )
    )
    os.system(
        "tar -zxf /tmp/{} --strip-components=1 --directory {}".format(
            file_name, igibson.g_dataset_path
        )
    )
...
def download_ig_dataset():
    ...
    os.system(
        "tar -zxf /tmp/{} --strip-components=1 --directory {}".format(
            file_name, igibson.ig_dataset_path
        )
    )
    # These datasets come as folders; in these folder there are scenes, so --strip-components are needed.
...
def change_data_path():
    with open(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "..", "global_config.yaml"
        )
    ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--download_assets", action="store_true", help="download assets file"
    )
    parser.add_argument(
        "--download_demo_data", action="store_true", help="download demo data Rs"
    )
    parser.add_argument(
        "--download_dataset", type=str, help="download dataset file given an URL"
    )
    parser.add_argument(
        "--download_ig_dataset", action="store_true", help="download iG Dataset"
    )
    parser.add_argument(
