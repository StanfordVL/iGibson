    ...
    for category_path in object_categories:
        ...
        if os.path.isdir(category_path):
            for object_instance in os.listdir(category_path):
                mvbb_meta = os.path.join(
                    category_path, object_instance, "misc", "mvbb_meta.json"
                )
                metadata_json = os.path.join(
                    category_path, object_instance, "misc", "metadata.json"
                )
                if os.path.exists(mvbb_meta):
                    ...
                    size = np.array(object_data["max_vert"]) - np.array(
                        object_data["min_vert"]
                    )
                    if object_data["ratio"] >= 0.9:
                        objects_to_process[idx] = {
                            ...
                            "urdf": os.path.join(
                                category_path,
                                object_instance,
                                f"{object_instance}.urdf",
                            ),
                            "mvbb_meta": mvbb_meta,
                            "base_link_offset": np.array(metadata["base_link_offset"]),
                            "size": size,
                            "euler": get_rpy_from_transform(
                                np.reshape(object_data["transform"], [3, 3])
                            ),
                        }
                        idx += 1
    new_urdfs = []
