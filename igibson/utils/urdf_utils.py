...
def get_base_link_name(tree):
    ...
    return [
        link.attrib["name"]
        for link in links
        if link.attrib["name"] not in children_links
    ][0]
...
def parse_urdf(tree):
    ...
    child_map = (
        {}
    )  # map from name of parent to list of names of children, joint names and types of connection
...
def splitter(parent_map, child_map, joint_map, single_child_link):
    ...
    for (joint_name, joint_tuple) in joint_map.items():
        ...
        if joint_tuple[2] == "floating":
            ...
            logging.info(
                "All children of the floating joint: " + " ".join(all_children)
            )
            ...
            for (
                parent
            ) in child_map:  # iterate all links that are parent of 1 or more joints
                # for each parent, get the list of children
                child_list = child_map[parent]
                if (
                    parent in all_children
                ):  # if the parent link was in the list of all children of the floating joint
...
def save_urdfs_without_floating_joints(tree, main_body_is_fixed, file_prefix):
        ...
        if joint_tuple[2] == "floating":
            ...
            while parent_name in parent_map.keys():
                ...
                joint_up = [
                    ...
                    if joint.find("child").attrib["link"] == parent_name
                ][0]
    ...
    for esd_key in extended_splitted_dict:
        xml_tree_parent = ET.ElementTree(
            ET.fromstring('<robot name="split_' + str(esd_key) + '"></robot>')
        )
...
def add_fixed_link(tree, link_name, link_info):
    ...
    ET.SubElement(
        joint,
        "origin",
        {"rpy": "%.4f %.4f %.4f" % tuple(rpy), "xyz": "%.4f %.4f %.4f" % tuple(xyz)},
    )
