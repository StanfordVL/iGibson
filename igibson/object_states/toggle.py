...
class ToggledOn(
    AbsoluteObjectState, BooleanState, LinkBasedStateMixin, TextureChangeStateMixin
):
    ...
    def _initialize(self):
        ...
        if self.link_id is not None:
            self.visual_marker_on = VisualMarker(
                rgba_color=[0, 1, 0, 0.5], radius=_TOGGLE_BUTTON_RADIUS
            )
            self.visual_marker_off = VisualMarker(
                rgba_color=[1, 0, 0, 0.5], radius=_TOGGLE_BUTTON_RADIUS
            )
    ...
    def _update(self):
        ...
        for robot in self.simulator.robots:
            for part_name, part in robot.parts.items():
                if part_name in ["left_hand", "right_hand"]:
                    if (
                        np.linalg.norm(
                            np.array(part.get_position())
                            - np.array(button_position_on_object)
                        )
                    ...
                    for finger in behavior_robot.FINGER_TIP_LINK_INDICES:
                        ...
                        if (
                            np.linalg.norm(
                                np.array(link_pos) - np.array(button_position_on_object)
                            )
                            < _TOGGLE_DISTANCE_THRESHOLD
                        ):
                            hand_in_marker = True
        ...
        show_marker = (
            self.visual_marker_on if self.get_value() else self.visual_marker_off
        )
        hidden_marker = (
            self.visual_marker_off if self.get_value() else self.visual_marker_on
        )
    ...
    def create_transformed_texture(
        ...
        brighten_texture(
            diffuse_tex_filename, diffuse_tex_filename_transformed, brightness=1.5
        )
