        ...
        for i in range(num_objects):
            ...
            while not found_placement:
                ...
                for attempt in range(num_place_attempts):
                    ...
                    if side == "right":
                        obj_x = np.random.uniform(
                            obj_width / 2.0, container.aabb[1][0] - obj_width / 2.0
                        )
                    elif side == "left":
                        obj_x = np.random.uniform(
                            container.aabb[0][0] + obj_width / 2.0, -obj_width / 2.0
                        )
                    else:
                        obj_x = np.random.uniform(
                            container.aabb[0][0] + obj_width / 2,
                    ...
                    obj_y = np.random.uniform(
                        y_start + (1 - attempt_ratio) * y_len, y_end
                    )
                    obj_z = np.random.uniform(
                        shelf_height + obj_height / 2,
                    ...
                    contact_points_with_shelf = pb.getContactPoints(
                        body_id, container.body_id
                    )
                ...
                if not found_placement:
                    if len(contact_points_with_shelf) > 0 and len(
                        contact_points_with_shelf
                    ) > len(contact_points_with_shelf):
                        print("CONTACT WITH SHELF AND OBJECTS")
                    elif len(contact_points_with_shelf) > 0 and len(
                        contact_points
                    ) == len(contact_points_with_shelf):
                        print("CONTACT WITH ONLY SHELF")
                    elif (
                        len(contact_points_with_shelf) == 0 and len(contact_points) > 0
                    ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Instantiate placements of objects on a shelf."
    )
    parser.add_argument(
        "objects_folder", type=str, help="Path to folder with objects to place on shelf"
    )
    parser.add_argument(
        "container_file", type=str, help="Path to urdf file of container with shelves"
    )
    parser.add_argument(
        "--shelf_num",
        type=int,
        default=0,
        help="Shelf number to place on (0 being lowest)",
    )
    parser.add_argument(
        "--side",
        type=str,
        default=None,
        help="Whether to put objects on only the left or right side of the shelf.",
    )
    parser.add_argument(
        "--num_objects", type=int, default=5, help="How many objects to place on shelf."
    )
    parser.add_argument(
        "--num_generate",
        type=int,
        default=5,
        help="How many different configurations of objects to create.",
    )
    parser.add_argument(
        "--count_start",
        type=int,
        default=0,
        help="First value to use for naming shelf placements.",
    )
    parser.add_argument(
        "--rot_randomization",
        type=float,
        default=0,
        help="How much to randomize object rotations.",
    )
    parser.add_argument(
        "--obj_scale", type=float, default=1.0, help="How much to scale objects."
    )
    parser.add_argument("--show_gui", action="store_true", help="Whether to show GUI.")
    args = parser.parse_args()
    generate_shelf_placements(
        args.objects_folder,
        args.container_file,
        args.shelf_num,
        args.num_generate,
