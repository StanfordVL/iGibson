        ...
        for i in range(num_objects):
            ...
            while not found_placement:
                ...
                for attempt in range(num_place_attempts):
                    ...
                    if side == "right":
                        ...
                        obj_x = np.random.uniform(
                            container.aabb[0][0] + obj_width / 2, container.aabb[1][0] - obj_width / 2
                        )
                    ...
                    obj_y = np.random.uniform(y_start + (1 - attempt_ratio) * y_len, y_end)
                    obj_z = np.random.uniform(
                        shelf_height + obj_height / 2, shelf_height + obj_height / 2.0 + 0.1 * attempt_ratio
                    )
                ...
                if not found_placement:
                    if len(contact_points_with_shelf) > 0 and len(contact_points_with_shelf) > len(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--side", type=str, default=None, help="Whether to put objects on only the left or right side of the shelf."
    )
    parser.add_argument("--num_objects", type=int, default=5, help="How many objects to place on shelf.")
    parser.add_argument(
        "--num_generate", type=int, default=5, help="How many different configurations of objects to create."
    )
