...
class ShelfObject(ArticulatedObject):
    def __init__(self, obj_fname, scale=1.0):
        ...
        if os.path.isfile(orientations_path):
            ...
            for i in range(len(orientations_dicts)):
                self.orientation_probs.append(
                    orientations_dicts[i]["prob"] / total_orientation_prob
                )
        else:
    ...
    def sample_orientation(self, Z_rotation_randomization=0):
        ...
        orientation = self.orientations[
            np.random.choice(len(self.orientations), p=self.orientation_probs)
        ]
        if Z_rotation_randomization == 0:
            return orientation
        rand_amount = np.random.uniform(
            -Z_rotation_randomization, Z_rotation_randomization
        )
...
class ContainerObjectsEnv(object):
    def __init__(self, path_to_cfg="cfg/container_env.yaml", show_gui=False):
        ...
        # set up renderer
        self.adjust_camera(
            [self.camera_x, self.camera_y, self.camera_height], [0, 0, 0], [-1, 0, 0]
        )
    ...
    def get_renderer_segmask(self, selected_object_id, with_occlusion=True):
        ...
        for instance in all_instances:
            if (
                ...
                and instance.pybullet_uuid == selected_object_id
            ):
                self.simulator.renderer.instances.append(instance)
    ...
    def set_camera_point_at(self, position, randomize=False):
        camera_eye_position = np.asarray(
            [position[0], position[1] - 0.3, position[2] + 0.1]
        )
    ...
    def gentle_drop(self, body_id, threshold=0.1):
        ...
        for _ in range(100000):
            ...
            if (
                np.linalg.norm(lvel) < threshold * 0.5
                and np.linalg.norm(avel) < threshold
            ):
                return True
