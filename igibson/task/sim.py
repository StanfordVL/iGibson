...
class StaticObject(ArticulatedObject):
    ...
    def _load(self):
        body_id = pb.loadURDF(
            self.filename, globalScaling=self.scale, useFixedBase=1, flags=pb.URDF_USE_MATERIAL_COLORS_FROM_MTL
        )
...
class ShelfObject(ArticulatedObject):
    ...
    def sample_orientation(self, Z_rotation_randomization=0):
        ...
        orientation = self.orientations[np.random.choice(len(self.orientations), p=self.orientation_probs)]
...
class ContainerObjectsEnv(object):
    def __init__(self, path_to_cfg="cfg/container_env.yaml", show_gui=False):
        config = {}
        ...
        if show_gui:
            ...
            self.simulator = Simulator(image_width=self.image_width, image_height=self.image_height, mode="headless")
            # timestep=0.001)
    ...
    def adjust_camera(self, camera_eye_position, camera_target_position, camera_up_vector, randomize=False):
        if randomize:
            ...
            self.simulator.renderer.set_camera(camera_eye_position, camera_target_position, camera_up_vector)
    ...
    def get_renderer_segmask(self, selected_object_id, with_occlusion=True):
        ...
        for instance in all_instances:
            if (
                ...
                and instance.pybullet_uuid == selected_object
            ):
    ...
    def set_camera_point_at(self, position, randomize=False):
        ...
        self.adjust_camera(camera_eye_position, camera_target_position, CAMERA_UP_VECTOR, randomize=randomize)
        self.simulator.sync()

    def get_observation(self, segmak_object_id=None, visualize=False, save=False, demo=False, randomize_camera=False):
    ...
    def gentle_drop(self, body_id, threshold=0.1):
        ...
        for _ in range(100000):
            ...
            if np.linalg.norm(avel) > threshold:
                ...
                new_avel = avel
            pb.resetBaseVelocity(objectUniqueId=body_id, angularVelocity=list(new_avel), linearVelocity=list(new_lvel))
