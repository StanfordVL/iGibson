...
def parse_args():
    ...
    parser.add_argument(
        "--scene_id", type=str, choices=scene_choices, required=True, help="Scene id"
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    parser.add_argument(
        "--task_id", type=int, help="PDDL integer ID, matching suffix of pddl."
    )
    parser.add_argument(
        "--max_trials",
        type=int,
...
def restore_scene(igtn_task, state_id, num_body_ids, num_particle_systems):
    ...
    igtn_task.simulator.particle_systems = igtn_task.simulator.particle_systems[
...
def main():
    ...
    if args.task is not None and args.task_id is not None:
        ...
        all_tasks = []
        all_task_ids = []
        condition_dir = os.path.join(os.path.dirname(bddl.__file__), "task_conditions")
    ...
    for task, task_id in zip(all_tasks, all_task_ids):
        if (
            task not in remove_tasks
            and task in activity_to_scenes
            and scene_id in activity_to_scenes[task]
        ):
            tasks.append(task)
    ...
    simulator = Simulator(
        mode="headless", image_width=400, image_height=400, rendering_settings=settings
    )
    scene_kwargs = {}
        ...
        for task_id in task_ids:
            ...
            for init_id in range(num_initializations):
                ...
                for _ in range(num_trials):
                    ...
                    igtn_task.object_scope["agent.n.01_1"] = igtn_task.agent.parts[
                        "body"
                    ]
                    accept_scene, _ = igtn_task.check_scene()
                    if not accept_scene:
                        restore_scene(
                            igtn_task, state_id, num_body_ids, num_particle_systems
                        )
                        continue
                ...
                if accept_scene:
                    ...
                    igtn_task.scene.save_modified_urdf(urdf_path, sim_obj_to_pddl_obj)
                    restore_scene(
                        igtn_task, state_id, num_body_ids, num_particle_systems
                    )
