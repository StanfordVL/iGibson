...
class iGTNTask(BEHAVIORActivityInstance):
    def __init__(self, atus_activity, task_instance=0, predefined_problem=None):
        ...
        super().__init__(
            atus_activity,
            activity_definition=task_instance,
    ...
    def check_scene(self):
            ...
            for obj_inst in room_type_to_obj_inst[room_type]:
                ...
                for room_inst in self.scene.room_sem_name_to_ins_name[room_type]:
                    if obj_cat == FLOOR_SYNSET:
                        ...
                        scene_objs = [
                            obj for obj in room_objs if obj.category in categories
                        ]
                    if len(scene_objs) != 0:
                        room_type_to_scene_objs[room_type][obj_inst][
                            room_inst
        ...
        for room_type in room_type_to_scene_objs:
            ...
            if len(room_inst_satisfied) == 0:
                ...
                for obj_inst in room_type_to_scene_objs[room_type]:
                    error_msg += (
                        "{}: ".format(obj_inst)
                        + ", ".join(room_type_to_scene_objs[room_type][obj_inst].keys())
                        + "\n"
                    )
                logging.warning(error_msg)
                feedback["init_success"] = "no"
        ...
        for obj_cat in self.objects:
            ...
            for obj_inst in self.objects[obj_cat]:
                ...
                if category == "pop" and self.atus_activity in [
                ...
                if is_sliceable:
                    ...
                    object_parts = []
                    assert (
                        "object_parts" in simulator_obj.metadata
                    ), "object_parts not found in metadata: [{}]".format(model_path)
    ...
    def import_agent(self):
        ...
        assert (
            len(self.simulator.robots) == 1
        ), "Error, multiple agents is not currently supported"
        ...
        if not self.online_sampling and self.scene.agent != {}:
            agent.parts["body"].set_base_link_position_orientation(
                self.scene.agent["BRBody_1"]["xyz"],
                quat_from_euler(self.scene.agent["BRBody_1"]["rpy"]),
            )
            agent.parts["left_hand"].set_base_link_position_orientation(
                self.scene.agent["left_hand_1"]["xyz"],
                quat_from_euler(self.scene.agent["left_hand_1"]["rpy"]),
            )
            agent.parts["right_hand"].set_base_link_position_orientation(
                self.scene.agent["right_hand_1"]["xyz"],
                quat_from_euler(self.scene.agent["right_hand_1"]["rpy"]),
            )
            agent.parts["left_hand"].ghost_hand.set_base_link_position_orientation(
                self.scene.agent["left_hand_1"]["xyz"],
                quat_from_euler(self.scene.agent["left_hand_1"]["rpy"]),
            ...
            agent.parts["eye"].set_base_link_position_orientation(
                self.scene.agent["BREye_1"]["xyz"],
                quat_from_euler(self.scene.agent["BREye_1"]["rpy"]),
            )
            ...
            for obj_inst in self.object_scope:
                ...
                if "floor.n.01" in obj_inst:
                    for _, sim_obj in self.scene.objects_by_name.items():
                        if (
                            sim_obj.bddl_object_scope is not None
                            and obj_inst in sim_obj.bddl_object_scope
                        ):
    ...
    def sample(self, kinematic_only=False):
        ...
        for condition in self.initial_conditions:
            if not isinstance(condition.children[0], Negation) and not isinstance(
                condition.children[0], AtomicFormula
            ):
                logging.warning(
                    (
            ...
            if kinematic_only:
                if isinstance(condition.children[0], Negation):
                    if (
                        condition.children[0].children[0].STATE_NAME
                        not in KINEMATICS_STATES
                    ):
                        continue
                else:
            ...
            for scene_obj in self.non_sampleable_object_scope[room_type]:
                ...
                for room_inst in self.non_sampleable_object_scope[room_type][scene_obj]:
                    for obj in self.non_sampleable_object_scope[room_type][scene_obj][
                        ...
                        if (
                            room_inst
                            not in scene_object_scope_filtered[room_type][scene_obj]
                        ):
                            scene_object_scope_filtered[room_type][scene_obj][
                                room_inst
                            ] = []
        ...
        for room_type in scene_object_scope_filtered:
            ...
            if len(room_inst_satisfied) == 0:
                ...
                error_msg += (
                    "The following are the initial condition sampling history:\n"
                )
                ...
                if self.should_debug_sampling:
                    self.debug_sampling(
                        scene_object_scope_filtered, non_sampleable_obj_conditions
                    )
                return False, feedback
            ...
            for obj_inst in scene_object_scope_filtered[room_type]:
                scene_object_scope_filtered[room_type][obj_inst] = {
                    key: val
                    for key, val in scene_object_scope_filtered[room_type][
                        obj_inst
                    ].items()
                    if key in room_inst_satisfied
                }
        ...
        for room_type in scene_object_scope_filtered:
            ...
            for room_inst in room_insts:
                ...
                for obj_inst in scene_object_scope_filtered[room_type]:
                    obj_inst_to_obj_per_room_inst[
                        obj_inst
                    ] = scene_object_scope_filtered[room_type][obj_inst][room_inst]
                top_nodes = []
            ...
            if not success:
                ...
                error_msg += (
                    "The following are the initial condition matching history:\n"
                )
                error_msg += "\n".join(init_mbm_log)
                logging.warning(error_msg)
                feedback["init_success"] = "no"
        ...
        logging.warning(
            ("number of ground_goal_state_options", len(self.ground_goal_state_options))
        )
        ...
        for goal_condition_set in self.ground_goal_state_options[
            :num_goal_condition_set_to_test
        ]:
                ...
                for scene_obj in scene_object_scope_filtered[room_type]:
                    ...
                    for room_inst in scene_object_scope_filtered[room_type][scene_obj]:
                        for obj in scene_object_scope_filtered[room_type][scene_obj][
                            ...
                            if (
                                room_inst
                                not in scene_object_scope_filtered_goal_cond[room_type][
                                    scene_obj
                                ]
                            ):
                                scene_object_scope_filtered_goal_cond[room_type][
            ...
            for room_type in scene_object_scope_filtered_goal_cond:
                ...
                if len(room_inst_satisfied) == 0:
                    ...
                    error_msg += (
                        "The following are the goal condition sampling history:\n"
                    )
                ...
                for obj_inst in scene_object_scope_filtered_goal_cond[room_type]:
                    scene_object_scope_filtered_goal_cond[room_type][obj_inst] = {
                        key: val
                        for key, val in scene_object_scope_filtered_goal_cond[
                            room_type
                        ][obj_inst].items()
                        if key in room_inst_satisfied
                    }
            ...
            for room_type in scene_object_scope_filtered_goal_cond:
                # The same room instances will be shared across all scene obj in a given room type
                some_obj = list(
                    scene_object_scope_filtered_goal_cond[room_type].keys()
                )[0]
                room_insts = list(
                    scene_object_scope_filtered_goal_cond[room_type][some_obj].keys()
                )
                success = False
                goal_mbm_log = []
                # Loop through each room instance
                ...
                if not success:
                    ...
                    error_msg += (
                        "The following are the goal condition matching history:\n"
                    )
                    error_msg += "\n".join(goal_mbm_log)
                    logging.warning(error_msg)
                    goal_sampling_error_msgs.append(error_msg)
    ...
    def clutter_scene(self):
        ...
        existing_objects = [
            value for key, value in self.object_scope.items() if "floor.n.01" not in key
        ]
    ...
    def nextTo(self, objA, objB):
        ...
        :param objB: simulator object
        """
        objA_aabb, objB_aabb = get_aabb(objA.get_body_id()), get_aabb(
            objB.get_body_id()
        )
    ...
    def under(self, objA, objB):
        ...
        within = aabb_contains_point(
            get_aabb_center(objA_aabb)[:2], aabb2d_from_aabb(objB_aabb)
        )
