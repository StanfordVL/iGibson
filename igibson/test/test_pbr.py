...
def test_render_pbr():
    hdr_texture = os.path.join(
        igibson.ig_dataset_path, "scenes", "background", "quattro_canti_4k.hdr"
    )
    ...
    renderer = MeshRenderer(
        width=1024, height=1024, vertical_fov=90, rendering_settings=settings
    )
    ...
    renderer.set_camera(
        [1.49, 1.49, 1.49], [0, 0.05, 0.05], [0, 0, 1], cache=True
...
def test_render_pbr_optimized():
    ...
    Image.fromarray(
        (255 * np.concatenate(frame, axis=1)[:, :, :3]).astype(np.uint8)
    ).save("test_render_optimized.png")
    renderer.set_camera(
        [1.49, 1.49, 1.49], [0, 0.05, 0.05], [0, 0, 1], cache=True
    )  # simulate camera movement
    frame = renderer.render(modes=("optical_flow", "scene_flow"))
