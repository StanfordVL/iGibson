...
def test_vision_sensor():
    ...
    assert np.all(0 <= vision_obs["depth"]) and np.all(vision_obs["depth"] <= 1.0)
    ...
    assert (
        np.sum(np.abs(normal_norm - 1) > 0.1) / (env.image_height * env.image_width)
        < 0.05
    )

    assert vision_obs["seg"].shape == (env.image_height, env.image_width, 1)
    assert np.all(0 <= vision_obs["seg"]) and np.all(
        vision_obs["seg"] <= MAX_CLASS_COUNT
    )
...
def test_scan_sensor():
    ...
    assert scan_obs.shape == (
        scan_sensor.n_horizontal_rays,
