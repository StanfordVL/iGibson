...
def save_scaled_urdf(filename, avg_size_mass, obj_class):
            ...
            if parent_link_name in scales_in_lf and child_link_name not in scales_in_lf:
                ...
                if joint.attrib["type"] == "prismatic":
                    ...
                    limit.attrib["upper"] = str(
                        float(limit.attrib["upper"]) * scale_in_parent_lf[major_axis]
                    )
                    limit.attrib["lower"] = str(
                        float(limit.attrib["lower"]) * scale_in_parent_lf[major_axis]
                    )
    ...
    total_mass = (
        avg_size_mass["density"] * bounding_box[0] * bounding_box[1] * bounding_box[2]
    )
...
def save_scale_urdfs():
    ...
    for obj_class_dir in os.listdir(root_dir):
        ...
        for obj_inst_dir in os.listdir(obj_class_dir):
            ...
            main_urdf_file, scaled_bbxc_in_blf = save_scaled_urdf(
    ...
    for i, obj_class_dir in enumerate(sorted(os.listdir(root_dir))):
        ...
        for obj_inst_dir in os.listdir(obj_class_dir):
            ...
            for _ in range(step_per_sec):
                ...
                imgs.append(
                    Image.fromarray((frame[0][:, :, :3] * 255).astype(np.uint8))
                )
            ...
            for i in range(step_per_sec * 4):
                ...
                p.applyExternalForce(
                    object_id, link_id, unit_force * force_mag, hit_pos, p.WORLD_FRAME
                )
                s.step()
                # print(hit_pos)
                frame = s.renderer.render(modes=("rgb"))
...
def debug_renderer_scaling():
    s = Simulator(
        mode="gui", image_width=512, image_height=512, physics_timestep=1 / float(100)
    )
    scene = EmptyScene()
    s.import_scene(scene, render_floor_plane=True)
    urdf_path = (
        "/cvgl2/u/chengshu/ig_dataset_v5/objects/window/103070/103070_avg_size_0.urdf"
    )
