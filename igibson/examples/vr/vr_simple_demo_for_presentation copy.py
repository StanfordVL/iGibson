...
def get_third_view(s):
    ...
    frame = s.renderer.render(modes=("rgb"), render_normal_cam=True)
    bg = (frame[0][:, :, :3] * 255).astype(np.uint8)
...
def follow(selection="user", headless=False, short_exec=False):
    ...
    scene = InteractiveIndoorScene(
        "Wainscott_0_int", #load_object_categories=["walls", "floors", "door"]#, load_room_types=["kitchen"] "ceilings", 
    )
    ...
    listener.set_position_orientation([-0.6000, 5.2000, 0.750000], final)
