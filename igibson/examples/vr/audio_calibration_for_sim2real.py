...
def main(selection="user", headless=False, short_exec=False):
    ...
    path = "testing/"
    # os.mkdir(path)
    exp_config = "C:/Users/capri28/Documents/iGibson-dev/igibson/examples/vr/data/audiogoal_continuous/config/audiogoal_continuous_for_sim2real.yaml"
    ...
    delta = tf3d.quaternions.axangle2quat([0, 0, 1], np.pi)
    start = np.array([1.0, 0., 0., 0.])
    final = tf3d.quaternions.qmult(start, delta)
    final = [final[1], final[2],final[3],final[0]]
    print("final", final)
    env.robots[0].set_position_orientation([.0, -2.0, 0.2], final)

    
    ppo_trainer._setup_actor_critic_agent(env.observation_space, action_space=env.action_space)
    ckpt_dict = ppo_trainer.load_checkpoint(r"C:\Users\capri28\Documents\iGibson-dev\igibson\examples\vr\data\savi_task\ckpt.83.pth", map_location="cpu")
    ...
    while True:
        try:
            ...
            step += 1
            obs, reward, done, info= env.step(np.array([0.,0.]))#np.array(action[0].cpu().numpy()))
            ...
            depth_map = state["depth"].squeeze() * 255.0
            ...
            if len(trajs_x) == 0:
                trajs_y.append(map_pos[0] * env.config["image_width_video"]/env.scene.trav_map_size)
                trajs_x.append(map_pos[1] * env.config["image_width_video"]/env.scene.trav_map_size)
    ...
    image_size = env.config["image_width_video"] / 100
...
def plot_audio():
    ...
    print(spec.shape)
    plot_spectrogram(spec[:, :, 0], "testing/l_spec.png")
    plot_spectrogram(spec[:, :, 1], "testing/r_spec.png")
    plot_wave_form(data[:, 0], fname="testing/l_wave_form.png")
    plot_wave_form(data[:, 1], fname="testing/r_wave_form.png")
    print(np.amin(data), np.amax(data))

def concat_audios():
    audios = []
    for i in range(1,22):
        samplerate, data = wavfile.read('new_noise/test_' + str(i) + '.wav')
        print(i)
        audios.append(data)
    merged = np.concatenate(audios, axis=0)
    print(merged.shape)
    wavfile.write("new_noise.wav", 44100, merged)
