...
class TrueMapOdomNode:
    ...
    def __init__(self):
        ...
        self.ground_truth_topic = rospy.get_param(
            "~ground_truth_topic", "ground_truth_odom"
        )
        ...
        if self.publish_pose_in_map_marker:
            self.marker_pub = rospy.Publisher(
                "markers_true_pose", Marker, queue_size=10
            )

        # Publish the pose on a pseudo amcl ground truth topic
        self.pose_in_map_pub = rospy.Publisher(
            "amcl_pose_ground_truth", PoseWithCovarianceStamped, queue_size=10
        )
        ...
        while map_from_world_tup is None:
            ...
            try:
                self.tf_listener.waitForTransform(
                    self.map_frame, self.world_frame, rospy.Time(0), rospy.Duration(0.1)
                )
                ...
                self.T_map_from_world = tf.transformations.translation_matrix(
                    t_map_from_world
                )
                self.R_map_from_world = tf.transformations.quaternion_matrix(
                    q_map_from_world
                )
                ...
                if (rospy.Time.now() - last_check).to_sec() > 2.0:
                    rospy.logwarn(
                        "Waiting for {}->{} transform. Got exception {}.".format(
                            self.world_frame, self.map_frame, e
                        )
                    )
        ...
        rospy.loginfo(
            "Got static transform {}->{}".format(self.world_frame, self.map_frame)
        )

        self.gt_sub = rospy.Subscriber(
            self.ground_truth_topic, Odometry, self.ground_truth_callback
        )
    ...
    def ground_truth_callback(self, odometry_msg):
        ...
        R_world_from_baselink, T_world_from_baselink = self.make_R_t_from_pose(
            self.ground_truth_odom.pose.pose
        )
        T_map_from_baselink = tf.transformations.concatenate_matrices(
            self.T_map_from_world,
            self.R_map_from_world,
            T_world_from_baselink,
            R_world_from_baselink,
        )
        ...
        try:
            ...
            (
                t_baselink_from_odom,
                q_baselink_from_odom,
        ...
        self.tf_broadcaster.sendTransform(
            t_map, q_map, rospy.Time.now(), self.odom_frame, self.map_frame
        )
