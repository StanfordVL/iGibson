...
def main(random_selection=False, headless=False, short_exec=False):
    ...
    if gui == "ig" and control_mode == "teleop" and isinstance(robot, ManipulationRobot):
        if controller_choices["arm"] == "InverseKinematicsController":
            message = (
                "Due to lack of free keys to control, the combination ig GUI and teleop actions for a robot "
                "with base and arm controlled in Cartesian space is not available. Please, use the PyBullet "
                "GUI instead to test this combination."
            )
