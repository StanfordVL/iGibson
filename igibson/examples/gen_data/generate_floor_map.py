...
def gen_map(obj_filepath, mesh_dir, img_filename_format="floor_{}.png"):
    ...
    with open(os.path.join(mesh_dir, "floors.txt")) as f:
        ...
        for i_floor, floor in enumerate(floors):
            z = float(floor) + 0.5
            cross_section = meshcut.cross_section(
                vertices, faces, plane_orig=(0, 0, z), plane_normal=(0, 0, 1)
            )
            ...
            for item in cross_section:
                for i in range(len(item) - 1):
                    ...
                    cv2.line(
                        floor_map, (x1, y1), (x2, y2), color=(0, 0, 0), thickness=2
                    )
...
def fill_template(
    map_filepath, resolution, origin
...
def write_yaml(
    ...
    yaml_content = fill_template(
        map_img_filepath, resolution=resolution, origin=cur_origin_map_coord
    )
