...
files = [
    file for file in files if file.startswith(uuid) and file.endswith("jpg")
]  # select all texture files
grid_size = int(np.ceil(np.sqrt(len(files))))

target_texture_grid_size = int(
    sys.argv[3]
)  # select a target texture size, the raw size is 2048, choose between 512,1024
# or 2048 depending on how many images you have

combined_img = np.zeros(
    (grid_size * target_texture_grid_size, grid_size * target_texture_grid_size, 3)
)
files = sorted(files)
# print(files)
for num_file, file in tqdm(enumerate(files)):
    img = np.array(
        Image.open(os.path.join(data_path, file)).resize(
            (target_texture_grid_size, target_texture_grid_size)
        )
    )
    i = grid_size - 1 - num_file // grid_size
    j = num_file % grid_size
    combined_img[
        i * target_texture_grid_size : (i + 1) * target_texture_grid_size,
        j * target_texture_grid_size : (j + 1) * target_texture_grid_size,
        :,
    ] = img

Image.fromarray(combined_img.astype(np.uint8)).save(
    os.path.join(data_path, "combined.jpg")
)
