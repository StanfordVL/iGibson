...
def main():
    ...
    renderer = MeshRenderer(
        width=512, height=512, vertical_fov=70, rendering_settings=settings
    )
    ...
    for fn in os.listdir(model_path):
        if fn.endswith("obj"):
            renderer.load_object(
                os.path.join(model_path, fn), scale=[scale, scale, scale]
            )
    ...
    def change_dir(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            if down:
                ...
                r2 = np.array(
                    [
                        [np.cos(-dx), -np.sin(-dx), 0],
                        [np.sin(-dx), np.cos(-dx), 0],
                        [0, 0, 1],
                    ]
                )
    ...
    while True:
        ...
        cv2.imshow(
            "test", cv2.cvtColor(np.concatenate(frame, axis=1), cv2.COLOR_RGB2BGR)
        )
