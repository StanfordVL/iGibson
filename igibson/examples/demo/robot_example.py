...
def main():
    ...
    robots = []
    config = parse_config(
        os.path.join(igibson.example_config_path, "fetch_reaching.yaml")
    )
    fetch = Fetch(config)
    robots.append(fetch)
    ...
    config = parse_config(
        os.path.join(igibson.example_config_path, "locobot_point_nav.yaml")
    )
    locobot = Locobot(config)
    robots.append(locobot)

    config = parse_config(
        os.path.join(igibson.example_config_path, "turtlebot_point_nav.yaml")
    )
