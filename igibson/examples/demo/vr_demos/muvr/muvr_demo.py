...
RUN_SETTINGS = {"client": VrSettings(use_vr=False), "server": VrSettings(use_vr=True)}
...
def run_muvr(mode="server", host="localhost", port="8885"):
    ...
    for i in range(len(mass_list)):
        ...
        s.import_object(
            mustard, use_pbr=False, use_pbr_mapping=False, shadow_caster=True
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Multi-user VR demo that can be run in server and client mode."
    )
    parser.add_argument(
        "--mode", default="server", help="Mode to run in: either server or client"
    )
    parser.add_argument(
        "--host",
        default="localhost",
        help="Host to connect to - eg. localhost or an IP address",
    )
