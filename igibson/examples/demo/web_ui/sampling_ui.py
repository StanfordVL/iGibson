...
class ProcessPyEnvironment(object):
    ...
    def start(self):
        ...
        self._process = multiprocessing.Process(
            target=self._worker, args=(conn, self._env_constructor)
        )
...
class ToyEnv(object):
    def __init__(self):
        config = parse_config(
            os.path.join(igibson.example_config_path, "turtlebot_demo.yaml")
        )
...
class ToyEnvInt(object):
    ...
    def restore_scene(self):
        ...
        self.task.simulator.particle_systems = self.task.simulator.particle_systems[
            : self.num_particle_systems
        ]
    ...
    def sample(self, atus_activity, pddl):
        try:
            self.task.update_problem(atus_activity, "tester", predefined_problem=pddl)
...
def check_sampling():
    ...
    pddl = construct_full_pddl(
        atus_activity, "feasibility_check", object_list, init_state, goal_state
    )
    ...
    for i, info in enumerate(scenes_ids):
        ...
        feedback_instances.append(
            (
                feedback["init_success"],
                feedback["goal_success"],
...
scheduler.add_job(
    id=PERIODIC_CLEANUP_TASK_ID, func=periodic_cleanup, seconds=5, trigger="interval"
)
