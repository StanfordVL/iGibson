...
def sample_microwave_plates_apples(simulator):
    ...
    cabinet = URDFObject(
        filename=cabinet_filename, category="cabinet", scale=np.array([2.0, 2.0, 2.0])
    )
    ...
    for i in range(3):
        ...
        for j in range(3):
            apple = URDFObject(
                filename=apple_filename, category="apple", model_path=apple_dir
            )
...
def sample_boxes_on_shelf(simulator):
    shelf_dir = os.path.join(
        igibson.ig_dataset_path, "objects/shelf/1170df5b9512c1d92f6bce2b7e6c12b7/"
    )
