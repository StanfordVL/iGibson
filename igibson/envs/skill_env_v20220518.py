...
skill_object_offset_params = {
    ...
    5:  # push
        {
            'cabinet.n.01_1': [0.3, -0.8, 0.35, 1, 0, 0],  # dx, dy, dz
        },
    6:  # vis_pick
        {
...
class SkillEnv(gym.Env):
    ...
    def __init__(
        ...
        self.skip_arm_planning = False  # True
        self.is_grasping = False
        self.robot = self.env.robots[0]
        self.arm = self.robot.default_arm
        print("Using with the default arm: {}".format(self.arm))
    ...
    def step(self, action_index):
        ...
        if action[0] == 0:  # base move to
            self._navigate_to(action, params=params)

        elif action[0] == 1:  # arm pick
            self._pick(action, params=params)

        elif action[0] == 2:  # arm place
            self._place(action, params=params)

        elif action[0] == 3:  # arm toggle
            self._toggle(action, params=params)

        elif action[0] == 4:  # arm pull
            self._pull(action, params=params)

        elif action[0] == 5:  # arm push
            self._push(action, params=params)
            if self.print_log:
            ...
            if obj_id in id_ins_seg:
                self._navigate_to(action, params=params)
                self._pick(action, params=params[4:])
                if self.print_log:
            ...
            if obj_id in id_ins_seg:
                ...
                self._place(action, params=params[4:])
                if self.print_log:
            ...
            if self.print_log:
                ...
                pass
            if obj_id in id_ins_seg:
                self._pull(action, params=params[4:])
                if self.print_log:
            ...
            if self.print_log:
                ...
                pass
            if obj_id in id_ins_seg:
                self._push(action, params=params[4:])
                if self.print_log:
    ...
    def _navigate_to(self, action, params):
    ...
    def _pick(self, action, params):
        ...
        if action[1] in ['towel'] and self.config['task'] in ['cleaning_microwave_oven']:
            ...
            object_id = self.task_obj_list[action[1]].get_body_ids()[0]  # Assume single body objects
            robot_is_grasping = self.robot.is_grasping(candidate_obj=None)
            robot_is_grasping_obj = self.robot.is_grasping(candidate_obj=object_id)
            if robot_is_grasping == IsGraspingState.TRUE:
                if robot_is_grasping_obj == IsGraspingState.TRUE:
        ...
        if not skip_flag:
            ...
            pre_pick_path, interaction_pick_path = self.planner.plan_ee_pick(pick_place_pos, pre_grasping_distance=0.1,
            ...
            if self.visualize_arm_path_flag:
                ...
                if plan_full_pre_grasp_motion:
                    self.planner.visualize_arm_path(
                        pre_pick_path, arm=self.arm, reverse_path=True, grasped_obj_id=object_id,
    ...
    def _place(self, action, params):
        ...
        pre_drop_path, _ = self.planner.plan_ee_drop(
            ...
            obj_name=action[1] + '-' + self.config['task']
        )
        ...
        # At the end, close the hand
        self._execute_ungrasp()

        logger.info("Place action completed at {}".format(pick_place_pos))

    def _toggle(self, action, params):
        object_name = action[1]
    ...
    def _pull(self, action, params):
        ...
        self._execute_ee_path(approach_interaction_path, stop_on_contact=True)
        # At the end, close the hand
        logger.info("Executing grasp")
        self._execute_grasp()
        ...
        self._execute_ee_path(pull_interaction_path, while_grasping=True)
        # Then, open the hand
    ...
    def _push(self, action, params):
        ...
        self._execute_ee_path(push_interaction_path, stop_on_contact=False)
    ...
    def _execute_grasp(self):
        ...
        action[self.env.robots[0].controller_action_idx["gripper_" + self.arm]] = -1.0
    ...
    def _get_obj_in_hand(self):
        obj_in_hand_id = self.env.robots[0]._ag_obj_in_hand[self.arm]  # TODO(MP): Expose this interface.
