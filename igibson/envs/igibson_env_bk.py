        ...
        if "vision" in self.sensors:
            ...
            if len(self.config["VIDEO_OPTION"])!=0:
                if 'rgb' in state.keys():
                    ...
                    state["rgb"] = skimage.measure.block_reduce(state["rgb"], (self.image_height_video//self.image_height,self.image_height_video//self.image_height,1), np.mean)
                if 'depth' in state.keys():
                    state['depth_video'] = state['depth']
                    state["depth"] = skimage.measure.block_reduce(state["depth"], (self.image_height_video//self.image_height,self.image_height_video//self.image_height,1), np.mean)
