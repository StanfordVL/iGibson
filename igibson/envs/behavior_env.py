...
class BehaviorEnv(iGibsonEnv):
    ...
    def load_behavior_task_setup(self):
        ...
        self.predicate_reward_weight = self.config.get("predicate_reward_weight", 1.0)
        self.distance_reward_weight = self.config.get("distance_reward_weight", 1.0)
    ...
    def load_ig_task_setup(self):
        ...
        agent = BehaviorRobot(
            self.simulator, use_tracked_body_override=True, show_visual_head=True, use_ghost_hands=False
        )
    ...
    def get_child_frame_pose(self, ag_bid, ag_link):
        ...
        link_state = p.getLinkState(self.robots[0].parts["right_hand"].get_body_id(), PALM_LINK_INDEX)
    ...
    def reset(self, resample_objects=False):
        ...
        if self.sample_objs is not None:
            ...
            for predicate, objA, objB in self.sample_objs:
                for _ in range(10):
                    success = (
                        self.task.object_scope[objA]
                        .states[get_state_from_name(predicate)]
                        .set_value(self.task.object_scope[objB], new_value=True, use_ray_casting_method=True)
                    )
                    if success:
                        break
                assert success, "Sampling failed: {} {} {}".format(predicate, objA, objB)
                self.sample_objs_poses.append(self.task.object_scope[objA].get_position_orientation())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action_filter",
        "-af",
        choices=["navigation", "tabletop_manipulation", "magic_grasping", "mobile_manipulation", "all"],
