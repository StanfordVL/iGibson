...
class ParallelNavEnv(iGibsonEnv):
    ...
    def step(self, actions):
        ...
        """
        time_steps = [
            env.step(action, self._blocking) for env, action in zip(self._envs, actions)
        ]
...
class ProcessPyEnvironment(object):
    ...
    def start(self):
        ...
        self._process = multiprocessing.Process(
            target=self._worker, args=(conn, self._env_constructor, self._flatten)
        )
...
if __name__ == "__main__":
    config_filename = os.path.join(
        os.path.dirname(igibson.__file__), "test", "test.yaml"
    )
