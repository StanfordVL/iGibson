...
def spline_traj(previous_shortest_path, steps_without_spline):
    ...
    spline_functions = []
    spline_functions.append(UnivariateSpline(range(path_length), steps_without_spline[:, 0], k=3, s=0.1))
    spline_functions.append(UnivariateSpline(range(path_length), steps_without_spline[:, 1], k=3, s=0.1))
