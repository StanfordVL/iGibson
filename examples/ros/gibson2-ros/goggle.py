...
class Goggle:
    def __init__(self):
        ...
        self.depth = None
        self.image_pub = rospy.Publisher("/gibson_ros/camera_goggle/rgb/image",
                                         Image,
                                         queue_size=10)
        self.depth_pub = rospy.Publisher("/gibson_ros/camera_goggle/depth/image",
                                         Image,
                                         queue_size=10)
    ...
    def load_model(self):
        ...
        comp.load_state_dict(torch.load(os.path.join(assets_file_dir, "unfiller_256.pth")))
    ...
    def rgb_callback(self, data):
        ...
        goggle_img = (recon.data.clamp(0, 1).cpu().numpy()[0].transpose(1, 2, 0) * 255).astype(
            np.uint8)
