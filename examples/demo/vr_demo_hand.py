...
for i in range(3):
    bottle = YCBObject('008_pudding_box')
    s.import_object(bottle)
    _, org_orn = p.getBasePositionAndOrientation(bottle.body_id)
    bottle_pos = [1, 0 - 0.2 * i, 1]
    bottle_orn = [1, 1, 0, 0]
    p.resetBasePositionAndOrientation(bottle.body_id, bottle_pos, bottle_orn)

pudding_box = YCBObject('024_bowl')
s.import_object(pudding_box)
_, org_orn = p.getBasePositionAndOrientation(pudding_box.body_id)
pudding_box_pos = [1, -0.6, 1]
p.resetBasePositionAndOrientation(pudding_box.body_id, pudding_box_pos, org_orn)
 
cracker_box = YCBObject('003_cracker_box')
s.import_object(cracker_box)
_, org_orn = p.getBasePositionAndOrientation(cracker_box.body_id)
cracker_box_pos = [1, -1.0, 1]
p.resetBasePositionAndOrientation(cracker_box.body_id, cracker_box_pos, org_orn)

soup_can = YCBObject('005_tomato_soup_can')
s.import_object(soup_can)
_, org_orn = p.getBasePositionAndOrientation(soup_can.body_id)
soup_can_pos = [1, -1.2, 1]
p.resetBasePositionAndOrientation(soup_can.body_id, soup_can_pos, org_orn)
