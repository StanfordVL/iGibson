...
class ORCA(object):
    ...
    def predict(self, state, walls=[], obstacles=[], allow_backward_motion=False):
        """
        ...
        if self.sim is None:
            ...
            self.sim.addAgent(self_state.position, *params, self.radius + self_state.personal_space,
                              self_state.v_pref, self_state.velocity)
            
            for human_state in state.human_states:
                self.sim.addAgent(human_state.position, *params, human_state.radius + human_state.personal_space,
...
#                 new_vel = vector_orthogonal_to_goal * np.dot(orca_vel, vector_orthogonal_to_goal) / np.dot(vector_orthogonal_to_goal, vector_orthogonal_to_goal)
            ...
            action = ActionXY(vx=new_vel[0], vy=new_vel[1])
