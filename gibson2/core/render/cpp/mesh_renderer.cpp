...
void MeshRendererContext::release(){
...
void MeshRendererContext::clean_meshrenderer(std::vector<GLuint> texture1, std::vector<GLuint> texture2, std::vector<GLuint> fbo, std::vector<GLuint> vaos, std::vector<GLuint> vbos) {
...
void MeshRendererContext::init_material_instance(int shaderProgram, float instance_color, py::array_t<float> diffuse_color,
...
Texture MeshRendererContext::createTexture(GLenum target, int width, int height, GLenum internalformat, int levels) const
{
    Texture texture;
    texture.width  = width;
...
Texture MeshRendererContext::createTexture(const std::shared_ptr<class Image>& image, GLenum format, GLenum internalformat, int levels) const
{
...
PYBIND11_MODULE(MeshRendererContext, m) {
        ...
        pymodule.def("setup_pbr", &MeshRendererContext::setup_pbr, "setup pbr");
