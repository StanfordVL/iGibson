...
class SimpleMeshRenderer:
    ...
    def generate_grid(self):
        ...
        GL.glVertexAttribPointer(positionAttrib, 3, GL.GL_FLOAT, GL.GL_FALSE, 40,
                                 None)
    ...
    def load_object(self, obj_path):
        ...
        GL.glVertexAttribPointer(positionAttrib, 3, GL.GL_FLOAT, GL.GL_FALSE, 40,
                                 None)
        GL.glVertexAttribPointer(normalAttrib, 3, GL.GL_FLOAT, GL.GL_FALSE, 40,
                                 ctypes.c_void_p(12))
        # the last parameter is a pointer
        GL.glVertexAttribPointer(coordsAttrib, 4, GL.GL_FLOAT, GL.GL_FALSE, 40,
                                 ctypes.c_void_p(24))
    ...
    def render(self):
        ...
        if self.render_marker:
            ...
            GL.glUniformMatrix4fv(GL.glGetUniformLocation(self.shaderProgram_simple, 'V'), 1, GL.GL_TRUE, self.V)
            GL.glUniformMatrix4fv(GL.glGetUniformLocation(self.shaderProgram_simple, 'P'), 1, GL.GL_FALSE, self.P)
            GL.glDrawElements(GL.GL_LINES, self.gridsize, GL.GL_UNSIGNED_INT, np.arange(self.gridsize, dtype=np.int))
            GL.glBindVertexArray(0)
            GL.glUseProgram(0)
        ...
        for i in range(len(self.VAOs)):
            ...
            GL.glUniform3f(GL.glGetUniformLocation(self.shaderProgram, 'instance_color'), *self.colors[i])
