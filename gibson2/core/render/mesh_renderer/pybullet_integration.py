...
filename = sys.argv[1]
collisionId = p.createCollisionShape(p.GEOM_MESH, fileName=filename, meshScale=1, flags=p.GEOM_FORCE_CONCAVE_TRIMESH)
...
for i in range(20):
    ...
    boundaryUid2 = p.createMultiBody(basePosition = [x,y,z], baseMass = 0.1, baseCollisionShapeIndex = collisionId2, baseVisualShapeIndex = -1)
...
while True:
    ...
    for i, ids in enumerate(collision_obj_list):
        ...
        renderer.set_pose([pos[0],pos[1],pos[2],orn[-1],orn[0],orn[1],orn[2]], renderer_obj_ids[i]) #[x y z quat]
