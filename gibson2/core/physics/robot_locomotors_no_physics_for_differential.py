...
class LocomotorRobot(BaseRobot):
    ...
    def __init__(
        ...
        self.max_vel_linear = 0
        self.max_vel_angular = 0
    ...
    def apply_real_action(self, action):
        if self.control == 'torque':
            ...
            #linear_velocity = (3.0 * action[0] + self.max_linear_velocity) * 0.25  # m/s
            #print("AV:", action[0])
            #print("LV:", linear_velocity)
            
            orientation = self.robot_body.get_orientation()
            angle = quat2euler(orientation)[0]
            linear_velocity_x = linear_velocity * np.cos(angle)
            linear_velocity_y = linear_velocity * np.sin(angle)
            
            p.resetBaseVelocity(self.robot_ids[0], [linear_velocity_x, linear_velocity_y, 0], [0, 0, angular_velocity])
...
class TurtlebotDifferentialDrive(LocomotorRobot):
    mjcf_scaling = 1
    model_type = "URDF"
    default_scale = 1
