...
class Tiago_Dual(LocomotorRobot):
    def __init__(self, config, with_hand=False):
        ...
        self.gripper_dim = 2
        self.hand_dim = 3 + 5 + 4 * 7  # 3 base joints + 5 thumb joints + 7 finger joints
        ...
        action_dim = self.wheel_dim + self.torso_lift_dim\
                + self.head_dim + self.arm_left_dim\
                + self.arm_right_dim + self.gripper_dim\
                + (self.hand_dim if self.with_hand else self.gripper_dim)
        LocomotorRobot.__init__(self,
                                "tiago/tiago_dual_hand.urdf" if self.with_hand\
                                        else "tiago/tiago_dual.urdf",
    ...
    def set_up_continuous_action_space(self):
        self.action_high = np.array(
                ...
                ([self.hand_velocity] * self.hand_dim  if self.with_hand\
                        else [self.gripper_velocity] * self.gripper_dim)
        )
