...
class IGLogWriter(object):
    ...
    def create_data_map(self):
        ...
        self.data_map['frame_data'] = np.full((self.frames_before_write, 4), self.default_fill_sentinel, dtype=self.np_dtype)
        ...
        for bid in self.tracked_objects:
            ...
            handle['joint_state'] = np.full(
                (self.frames_before_write, self.joint_map[bid]), self.default_fill_sentinel)
        ...
        if self.vr_robot:
            self.data_map['agent_actions'] = {
                ...
                'left_hand_local': np.full((self.frames_before_write, 7), self.default_fill_sentinel, dtype=self.np_dtype),
                'right_hand_local': np.full((self.frames_before_write, 7), self.default_fill_sentinel, dtype=self.np_dtype),
                'left_hand_trigger_fraction': np.full((self.frames_before_write, 1), self.default_fill_sentinel, dtype=self.np_dtype),
                'right_hand_trigger_fraction': np.full((self.frames_before_write, 1), self.default_fill_sentinel, dtype=self.np_dtype),
            }
    ...
    def write_agent_data_to_map(self):
        ...
        self.data_map['agent_actions']['left_hand_local'][self.frame_counter][:3] = self.vr_robot.parts['left_hand'].local_pos
        self.data_map['agent_actions']['left_hand_local'][self.frame_counter][3:] = self.vr_robot.parts['left_hand'].local_orn


        self.data_map['agent_actions']['right_hand_local'][self.frame_counter][:3] = self.vr_robot.parts['right_hand'].local_pos
        self.data_map['agent_actions']['right_hand_local'][self.frame_counter][3:] = self.vr_robot.parts['right_hand'].local_orn
        self.data_map['agent_actions']['left_hand_trigger_fraction'][self.frame_counter] = self.vr_robot.parts['left_hand'].trigger_fraction
        self.data_map['agent_actions']['right_hand_trigger_fraction'][self.frame_counter] = self.vr_robot.parts['right_hand'].trigger_fraction
    ...
    def end_log_session(self):
        ...
        if self.log_status:
            print('IG LOGGER INFO: Ending log writing session after {} frames'.format(
                self.persistent_frame_count))
        self.hf.close()
...
class IGLogReader(object):
    ...
    def set_replay_camera(self, sim):
        ...
        sim.renderer.set_light_position_direction([right_cam_pos[0], right_cam_pos[1], 10], [
