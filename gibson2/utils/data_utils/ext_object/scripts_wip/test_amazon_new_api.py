...
def getDimsAndWeight(obj_list):
    ...
    specs = {obj : {"size":[-1,-1,-1], "mass":-1, "density":-1} for obj in obj_list}

    for query in obj_list:
        payload = {
            'MarketplaceId': market_id,
            'Query': query,
        }
        ...
        print(query)
        #print(obj)
        ### HACK to get around quota exceeded limit
        ...
        for item in obj['payload']['Items']:
            ...
            if 'ItemDimensions' in attribute_sets:
                ...
                if not has_valid_dim:
                    continue
                assert(item_dim['Height']['Units'] == 'inches' and
                        item_dim['Length']['Units'] == 'inches'and
                        item_dim['Width']['Units'] == 'inches'and
                        item_dim['Weight']['Units'] == 'pounds')
                item_dim_array = [
                    max(item_dim['Width']['value'], item_dim['Length']['value']) * 0.0254,
                    min(item_dim['Width']['value'], item_dim['Length']['value']) * 0.0254,
                    item_dim['Height']['value'] * 0.0254,
                    item_dim['Weight']['value'] *  0.45359237, #Conversion to m and kg
                ]
        ...
        for dim in dims:
            print(dim)
        medians = np.median(dims, axis=0)
        print(medians)
        specs[query]["density"] = medians[3] / np.prod(medians[0:3])
        specs[query]["enable_ag"] = 1
        specs[query]["mass"] = medians[3]
        specs[query]["size"] = medians[0:3].tolist()
...
if __name__ == '__main__':
    ...
    getDimsAndWeight(args.obj_list)
