...
def learn(
    ...
    lossandgrad = U.function([ob, ob_sensor, ac, atarg, ret, lrmult],
    ...
    while True:
        ...
        ob, ob_sensor, ac, atarg, tdlamret = seg["ob"], seg["ob_sensor"], seg["ac"], seg[
            "adv"], seg["tdlamret"]
        vpredbefore = seg["vpred"]    # predicted value function before udpate
        atarg = (atarg - atarg.mean()) / atarg.std()    # standardized advantage function estimate
        d = Dataset(dict(ob=ob, ob_sensor=ob_sensor, ac=ac, atarg=atarg, vtarg=tdlamret),
                    shuffle=not pi.recurrent)
        optim_batchsize = optim_batchsize or ob.shape[0]

        if hasattr(pi, "ob_rms"):
            pi.ob_rms.update(ob_sensor)    # update running mean/std for policy (sensor output)
        ...
        for _ in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                *newlosses, g = lossandgrad(batch["ob"], batch["ob_sensor"], batch["ac"],
        ...
        losses = []
        for batch in d.iterate_once(optim_batchsize):
            newlosses = compute_losses(batch["ob"], batch["ob_sensor"], batch["ac"], batch["atarg"],
