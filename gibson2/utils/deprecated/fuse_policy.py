...
class FusePolicy(object):
    ...
    def _init(self, ob_space, sensor_space, ac_space, hid_size, num_hid_layers, kind):
        ...
        if kind == 'small':    # from A3C paper
            ...
            x = tf.nn.relu(
                tf.layers.dense(x, 256, name='lin', kernel_initializer=U.normc_initializer(1.0)))
            ...
            x = U.flattenallbut0(x)
            x = tf.nn.relu(
                tf.layers.dense(x, 64, name='lin', kernel_initializer=U.normc_initializer(1.0)))
        else:
