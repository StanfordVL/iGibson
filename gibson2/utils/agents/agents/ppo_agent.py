...
class PPOAgent(tf_agent.TFAgent):
    ...
    def __init__(self,
        ...
        Args:
          ...
          optimizer: Optimizer to use for the agent._distribution
    ...
    def get_epoch_loss(self, time_steps, actions, act_log_probs, returns,
        ...
        if self._action_mask:
            action_mask_loss = tf.reduce_mean(tf.abs(current_policy_distribution.loc))
        else:
            action_mask_loss = tf.zeros_like(policy_gradient_loss)

        total_loss = (
                policy_gradient_loss + value_estimation_loss + l2_regularization_loss +
                entropy_regularization_loss + kl_penalty_loss + action_mask_loss)
    ...
    def _train(self, experience, weights):
        ...
        for i_epoch in range(self._num_epochs):
            with tf.name_scope('epoch_%d' % i_epoch):
                ...
                action_mask_losses.append(loss_info.extra.action_mask_loss)
        ...
        with tf.name_scope('Losses/'):
            ...
            total_action_mask_loss = tf.add_n(action_mask_losses)
            ...
            if self._action_mask:
                tf.compat.v2.summary.scalar(
                    name='action_mask_loss',
                    data=total_action_mask_loss,
                    step=self.train_step_counter)
