...
def parse_args():
    ...
    parser.add_argument('--frame_save_path', type=str, default='', help='Path to save frames (frame number added automatically, as well as .jpg extension)')
...
def main():
    ...
    vr_frame_save_path = None if args.frame_save_path == '' else args.frame_save_path
    vr_replay_settings.set_frame_save_path(vr_frame_save_path)

    # VR system settings
    s = Simulator(mode='vr', rendering_settings=vr_rendering_settings, vr_settings=vr_replay_settings)
    ...
    vr_reader = VRLogReader(args.vr_log_path, s, emulate_save_fps=False, log_status=False)

    satisfied_predicates_cached = {}
    while vr_reader.get_data_left_to_read():
        vr_reader.pre_step()
        igtn_task.simulator.step(print_stats=args.profile, forced_timestep=vr_reader.get_phys_step_n())
