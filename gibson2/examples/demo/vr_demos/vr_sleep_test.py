...
def main():
    ...
    for st in [5,10,15,20,25,30,35,40,45,50,55,60]:
        av_pre_vr_dur = []
        av_vr_dur = []
        for _ in range(100):
                pre_vr_dur, vr_dur = s.step_sleep_test(st)
                av_pre_vr_dur.append(pre_vr_dur)
                av_vr_dur.append(vr_dur)
        print('Sleep time: {}'.format(st))
        print('Average pre: {} and vr: {}'.format(np.mean(av_pre_vr_dur), np.mean(av_vr_dur)))
