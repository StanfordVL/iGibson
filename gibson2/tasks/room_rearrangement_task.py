...
class RoomRearrangementSimpleTask(BaseTask):
    def __init__(self, env):
        super(RoomRearrangementSimpleTask, self).__init__(env)
        self.task_randomization_freq = self.config.get(
            'task_randomization_freq', None,
        )
        if self.task_randomization_freq is None:
    ...
    def sample_initial_pose(self, env):
        ...
        initial_orn = np.array([0, 0, self.initial_orn_range[env.scene.scene_id]])
