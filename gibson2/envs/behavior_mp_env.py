...
class BehaviorMPEnv(BehaviorEnv):
                    ...
                    if isinstance(obj, URDFObject) and hasattr(obj, 'states') and object_states.AABB in obj.states:
                        ...
                        if volume < 0.2 * 0.2 * 0.2 and not obj.main_body_is_fixed: # say we can only grasp small objects
                            if np.linalg.norm(np.array(obj.get_position()) - np.array(self.robots[0].get_position())) < 2:
                                ...
                                logging.debug('PRIMITIVE: grasp {} success, obj in hand {}'.format(obj.name, self.obj_in_hand))
                            else:
                    ...
                    if isinstance(obj, URDFObject):
                        if np.linalg.norm(np.array(obj.get_position()) - np.array(self.robots[0].get_position())) < 2:
                            ...
                            result = sample_kinematics('onTop', self.obj_in_hand, obj, True, use_ray_casting_method=True,
                                                        max_trials=20, skip_falling=True)
                            if result:
                                logging.debug('PRIMITIVE: place {} ontop {} success'.format(self.obj_in_hand.name, obj.name))
                                ...
                                p.removeState(state)
                                logging.debug('PRIMITIVE: place {} ontop {} fail, sampling fail'.format(self.obj_in_hand.name, obj.name))

                        else:
                            logging.debug('PRIMITIVE: place {} ontop {} fail, too far'.format(self.obj_in_hand.name, obj.name))
                    else:
                        state = p.saveState()
                        ...
                        if result:
                            ...
                            logging.debug('PRIMITIVE: place {} ontop {} fail, sampling fail'.format(self.obj_in_hand.name,
                                                                                            obj.name))
                            p.removeState(state)
                    ...
                    if np.linalg.norm(np.array(obj.get_position()) - np.array(self.robots[0].get_position())) < 2:
                        if (hasattr(obj, 'states') and object_states.Open in obj.states
                            ...
                            result = sample_kinematics('inside', self.obj_in_hand, obj, True, use_ray_casting_method=True,
                                                       max_trials=20, skip_falling=True)
                            if result:
                                logging.debug('PRIMITIVE: place {} inside {} success'.format(self.obj_in_hand.name, obj.name))
                                pos = self.obj_in_hand.get_position()
                                orn = self.obj_in_hand.get_orientation()
                                self.place_obj(state, pos, orn, use_motion_planning=self.use_motion_planning)
                            else:
                                logging.debug('PRIMITIVE: place {} inside {} fail, sampling fail'.format(self.obj_in_hand.name, obj.name))
                                p.removeState(state)
                        else:
                            logging.debug('PRIMITIVE: place {} inside {} fail, need open not open'.format(self.obj_in_hand.name,
                                                                                             obj.name))
                    else:
                        logging.debug('PRIMITIVE: place {} inside {} fail, too far'.format(self.obj_in_hand.name, obj.name))
    ...
    def grasp_obj(self, obj, use_motion_planning=False):
        if use_motion_planning:
            ...
            plan = plan_hand_motion_br(robot=self.robots[0],
                                       obj_in_hand=None,
        ...
        if isinstance(obj, URDFObject):
            ...
            for distance in distance_to_try:
                for _ in range(20):
                    ...
                    pos = [obj_pos[0] + distance * np.cos(yaw), obj_pos[1] + distance * np.sin(yaw),
            ...
            if use_motion_planning:
                ...
                if plan is not None:
                    if self.mode != 'headless':
                        ...
                        self.robots[0].set_position_orientation(valid_position[0],
                                                                p.getQuaternionFromEuler(valid_position[1]))
                    return True
                else:
                    self.robots[0].set_position_orientation(original_position, original_orientation)
                    return False
