from gibson2.data.datasets import ViewDataSet3D, get_model_path
from gibson2.core.render.pcrender import PCRenderer
from gibson2.core.render.profiler import Profiler
from gibson2.envs.env_bases import BaseEnv
from gibson2.envs.env_utils import *
import gibson2
from gym import error
from gym.utils import seeding
from transforms3d import quaternions
from gibson2.envs.env_ui import *
...
DEFAULT_FRAMESKIP = 4
DEFAULT_DEBUG_CAMERA = {'yaw': 30, 'distance': 2.5, 'pitch': -35, 'z_offset': 0}
...
class BaseRobotEnv(BaseEnv):
    ...
    def _step(self, a):
        ...
        if self.gui:
            ...
            p.resetDebugVisualizerCamera(dist, self.tracking_camera['yaw'],
                                         self.tracking_camera['pitch'], pos)
        ...
        return observations, sum(self.rewards), bool(done), dict(eye_pos=eye_pos,
                                                                 eye_quat=eye_quat,
                                                                 episode=episode)
            ...
            if len(curr_pos) <= p.MAX_RAY_INTERSECTION_BATCH_SIZE:
                ...
                while (curr_i < len(curr_pos)):
                    ...
                    collisions = collisions + list(
                        p.rayTestBatch([eye_pos] * (curr_n - curr_i), curr_pos[curr_i:curr_n]))
                    curr_i = curr_n
...
class CameraRobotEnv(BaseRobotEnv):
    ...
    def __init__(self, config, gpu_idx, scene_type, tracking_camera):
        ...
        self._require_rgb = 'rgb_filled' in self.config["output"] or "rgb_prefilled" in self.config[
            "output"]
    ...
    def setup_camera_multi(self):
        ...
        dr_path = osp.join(osp.dirname(osp.abspath(gibson2.__file__)), 'core', 'channels',
                           'depth_render')
        cur_path = os.getcwd()
        os.chdir(dr_path)

        render_main = "./depth_render --GPU {} --modelpath {} -w {} -h {} -f {} -p {}".format(
            self.gpu_idx, self.model_path, self.windowsz, self.windowsz,
            self.config["fov"] / np.pi * 180, self.port_depth)
        render_norm = "./depth_render --GPU {} --modelpath {} -n 1 -w {} -h {} -f {} -p {}".format(
            self.gpu_idx, self.model_path, self.windowsz, self.windowsz,
            self.config["fov"] / np.pi * 180, self.port_normal)
        render_semt = "./depth_render --GPU {} --modelpath {} -t 1 -r {} -c {} -w {} -h {} -f {} -p {}".format(
            self.gpu_idx, self.model_path, self._semantic_source, self._semantic_color,
            self.windowsz, self.windowsz, self.config["fov"] / np.pi * 180, self.port_sem)
...
class SemanticRobotEnv(CameraRobotEnv):
    ...
    def setup_semantic_parser(self):
        ...
        cur_path = os.getcwd()
        os.chdir(dr_path)
        load_semantic = "./semantic --modelpath {} -r {} ".format(self.model_path,
        ...
        if self._semantic_source == 2:
            _, semantic_ids, _ = get_segmentId_by_name_MP3D(
                osp.join(self.model_path, "semantic.house"), "chair")
        elif self._semantic_source == 1:
            _, semantic_ids, _ = get_segmentId_by_name_2D3DS(
                osp.join(self.model_path, "semantic.mtl"), osp.join(self.model_path,
                                                                    "semantic.obj"), "chair")
    ...
    def get_close_semantic_pos(self, dist_max=1.0, orn_max=np.pi / 5):
        ...
        return [
            i for i in range(self.semantic_pos.shape[0])
            if dists[i] < dist_max and orns[i] < orn_max
        ]
