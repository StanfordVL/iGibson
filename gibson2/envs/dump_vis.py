...
def get_coffee_results(env, states_f):
    ...
    return mug_pos, mug_fill_success
...
def vis_all(states_folder, video_path):
    ...
    for epoch, h5_path in zip(file_epoch, all_files):
        ...
        mug_pos, mug_fill_success = get_coffee_results(env, f)
        f.close()
        im = draw_mug_positions(mug_pos, mug_fill_success, epoch=epoch)
...
def vis_image(hdf5_file, image_path):
    ...
    mug_pos, mug_fill_success = get_coffee_results(env, f)
    f.close()
    im = draw_mug_positions(mug_pos, mug_fill_success, epoch=0, save_path=image_path)
