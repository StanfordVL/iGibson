    ...
    def get_state(self, collision_links=[]):
        ...
        if 'scan' in self.output:
            if self.config['robot'] == 'Turtlebot':
                ...
                min_laser_dist = 0.1
                laser_link_name = 'scan_link'
            elif self.config['robot'] == 'JR2_Kinova' or self.config['robot'] == 'JR2':
                # SICK TiM571-2050101 Laser Range Finder
                laser_linear_range = 25.0
                laser_angular_range = 220.0
                min_laser_dist = 0.1
    ...
    def get_ped_time_to_collision(self):
        ...
        for pos, vel in zip(ped_pos, ped_vel):
            ...
            if time_to_collision < 0:
                ...
                time_to_collision = 1.0 - np.tanh(time_to_collision)
...
#                     p.setCollisionFilterPair(pedestrian_id, ignore_body_id, ignore_link_id, 0, 0)
    ...
    def step(self, action):
        ...
        for i, human in enumerate(self.humans):
            ...
            if l2_distance(np.array(human.get_goal_position())[0:2], self.pedestrians[i].get_position()[0:2]) < self.pedestrian_dist_tol:
                prob = random.random()
...
#                 # get time-to-collision for each pedestrian
#                 pedestrian_ttc = np.array(self.get_ped_time_to_collision())
#                 
#                 ttc_cost = -0.01 * np.sum(pedestrian_ttc[pedestrian_ttc > 0])
#                                                 
#                 reward += ttc_cost
    ...
    def reset_initial_and_target_pos(self):
        ...
        # Choose new start position and goal location for robot
        reset_complete = False
        while not reset_complete:
            if random.uniform(0, 1) < 0.5:
                init = True
                center_x = self.agent_init['center_x']
                center_y = self.agent_init['center_y']
            else:
                init = False
                center_x = self.agent_target['center_x']
                center_y = self.agent_target['center_y']
            ...
            pos = np.array([np.random.uniform(center_x - self.agent_x_range_radius, center_x + self.agent_x_range_radius), np.random.uniform(center_y - self.agent_y_range_radius, center_y + self.agent_y_range_radius), 0.0])
            ...
            for _ in range(max_trials):  # if initial and target positions are < 1 meter away from each other, reinitialize
                ...
                self.current_target_position = np.array([np.random.uniform(center_x - self.agent_x_range_radius, center_x + self.agent_x_range_radius), np.random.uniform(center_y- self.agent_y_range_radius, center_y + self.agent_y_range_radius), 0.0])
...
class NavigateRandomObstaclesEnv(NavigateEnv):
    def __init__(self,
        ...
        for i in range(len(self.walls)):
            wall = self.walls[i]
            box = BoxShape(pos=[wall[0][0], wall[0][1], wall[0][2]], dim=[wall[1][0]*2, wall[1][1]*2, wall[1][2]*2])            
            self.obstacle_ids.append(self.simulator.import_object(box))
