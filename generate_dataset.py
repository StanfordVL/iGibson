...
class GenerateWayPoints(object):
    ...
    def get_splined_steps(self, trajectory):
        spline_parameter, _ = splprep(
            [trajectory[:, 0], trajectory[:, 1]], s=0.2)
        ...
        for trajectory in self.scene_trajectories:
            ...
            for i in range(1, len(trajectory)):
                ...
                if not first_iteration:
                    trajectory_waypoints = np.append(
                        trajectory_waypoints, shortest_path_steps[1:], axis=0)
                else:
...
class GenerateDataset(object):
    ...
    def generate_waypoints(self):
        ...
        for scene in scene_list:
            waypoint_generator = GenerateWayPoints(
                scene, self.num_trajectories)
